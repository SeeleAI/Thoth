...
async function seedRestartHome(): Promise<SeededRestartHome> {
  const thothHome = mkdtempSync(path.join(tmpdir(), "thoth-playwright-restart-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "thoth-playwright-restart-cwd-"));
  const projectsDir = path.join(thothHome, "projects");
  const agentDir = path.join(thothHome, "agents", projectDirNameFromCwd(cwd));
...
async function startRestartDaemon(input: {
  ...
  const child = spawn(tsxBin, ["scripts/supervisor-entrypoint.ts", "--dev"], {
    ...
    env: {
      ...process.env,
      THOTH_HOME: input.thothHome,
      THOTH_SERVER_ID: SERVER_ID,
...
test.describe("Workspace model restart regressions", () => {
  test("browser-created same-cwd workspace preserves restarted agent status and migrated tab ownership", async ({
    ...
    const daemon = await startRestartDaemon({ thothHome: seeded.thothHome, origin });
