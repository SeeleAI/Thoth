...
export interface ThothHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveThothHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkThothHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<ThothHomeMetadataForkResult> {
  const sourceHome = resolveThothHomePath(input.sourceHome);
  const targetHome = resolveThothHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Thoth metadata onto the same THOTH_HOME.");
  }
