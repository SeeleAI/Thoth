...
function destroyWebviewSelector(webview: ElectronWebview): void {
  void executeWebviewJavaScript(
    webview,
    "if(window.__thothSelector) window.__thothSelector.destroy();",
...
function clearWebviewSelector(webview: ElectronWebview): void {
  void executeWebviewJavaScript(
    webview,
    "if(window.__thothSelector) window.__thothSelector.destroy(); window.__thothSelectorResult = null;",
  ).catch(ignoreWebviewJavaScriptError);
}
    ...
    void (async () => {
      try {
        const raw = await executeWebviewJavaScript(
          webview,
          "JSON.stringify(window.__thothSelectorResult || null)",
        );
        ...
        onDone();
        await executeWebviewJavaScript(webview, "window.__thothSelectorResult = null;");
  ...
  const startElementSelector = useCallback(() => {
    ...
    const js = `
      (function() {
        ...
        style.textContent = [
          '.__thoth-hover { outline: 2px solid #3b82f6 !important; outline-offset: 2px !important; cursor: crosshair !important; }',
          '.__thoth-select-mode, .__thoth-select-mode * { cursor: crosshair !important; pointer-events: auto !important; user-select: none !important; }',
          '.__thoth-select-mode *, .__thoth-select-mode *::before, .__thoth-select-mode *::after { animation: none !important; transition: none !important; }',
          '.__thoth-select-mode a, .__thoth-select-mode button, .__thoth-select-mode input, .__thoth-select-mode select, .__thoth-select-mode textarea, .__thoth-select-mode [role="button"], .__thoth-select-mode [onclick] { pointer-events: none !important; }',
          '.__thoth-select-mode iframe, .__thoth-select-mode video, .__thoth-select-mode audio { pointer-events: none !important; }',
        ].join('\\n');
        document.head.appendChild(style);
        document.documentElement.classList.add('__thoth-select-mode');
        var last = null;
        function onMove(e) {
          e.preventDefault();
          e.stopPropagation();
          if (last) last.classList.remove('__thoth-hover');
          e.target.classList.add('__thoth-hover');
          last = e.target;
        }
        ...
        function onClick(e) {
          ...
          destroy();
          window.__thothSelectorResult = result;
        }
        function onKey(e) {
          if (e.key === 'Escape') { destroy(); window.__thothSelectorResult = { __cancelled: true }; }
        }
        ...
        function destroy() {
          ...
          document.documentElement.classList.remove('__thoth-select-mode');
          if (last) last.classList.remove('__thoth-hover');
          style.remove();
          window.__thothSelector = null;
        }
        ...
        window.__thothSelector = { destroy: destroy };
      })()
    `;
