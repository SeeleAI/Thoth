...
function makeHost(): HostProfile {
  return {
    ...
    connections: [
      ...
      {
        id: "socket:/tmp/thoth-secret.sock",
        type: "directSocket",
        path: "/tmp/thoth-secret.sock",
      },
      {
        id: "pipe:\\\\.\\pipe\\thoth-secret",
        type: "directPipe",
        path: "\\\\.\\pipe\\thoth-secret",
      },
    ],
  };
}
...
describe("app diagnostics report", () => {
  test("formats connection rows without raw connection details", () => {
    ...
    expect(report).not.toContain("/tmp/thoth-secret.sock");
  ...
  test("redacts saved connection secrets from collected daemon and desktop text", () => {
    ...
    const redacted = redactAppDiagnosticReport(
      [
        ...
        "/tmp/thoth-secret.sock",
        "\\\\.\\pipe\\thoth-secret",
        "password=tcp-password",
        "thoth://pairing-secret",
      ].join("\n"),
      [host],
    );
    ...
    expect(redacted).not.toContain("/tmp/thoth-secret.sock");
    expect(redacted).not.toContain("\\\\.\\pipe\\thoth-secret");
