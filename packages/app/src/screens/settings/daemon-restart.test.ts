...
describe("restartDaemonFromSettings", () => {
  it("restarts a local Desktop-managed daemon through Desktop IPC", async () => {
    const { calls, deps } = makeDeps();

    await restartDaemonFromSettings(" local-desktop ", "settings_restart_local", deps);

    expect(calls).toEqual(["desktop-status", "desktop-settings", "desktop-restart"]);
  });

  it("restarts remote hosts through daemon RPC without reading Desktop settings", async () => {
    const { calls, deps } = makeDeps({
      desktopSettingsError: new Error("Unreadable Desktop settings"),
    });

    await restartDaemonFromSettings("remote-host", "settings_restart_remote", deps);

    expect(calls).toEqual(["desktop-status", "rpc-restart:settings_restart_remote"]);
  });
  ...
  it("keeps manually managed local daemons on the RPC path", async () => {
    ...
    expect(calls).toEqual(["desktop-status", "rpc-restart:settings_restart_local"]);
  });

  it("keeps the RPC path when built-in daemon management is disabled", async () => {
    const { calls, deps } = makeDeps({
      desktopSettings: { daemon: { manageBuiltInDaemon: false } },
    });
    ...
    expect(calls).toEqual([
      "desktop-status",
      "desktop-settings",
      "rpc-restart:settings_restart_local",
    ]);
  });

  it("uses the RPC path outside Electron", async () => {
    const { calls, deps } = makeDeps({ isElectron: false });

    await restartDaemonFromSettings("local-desktop", "settings_restart_local", deps);

    expect(calls).toEqual(["rpc-restart:settings_restart_local"]);
  });
  ...
  it("surfaces Desktop restart failures without falling back to worker recycle", async () => {
    ...
    await expect(
      restartDaemonFromSettings("local-desktop", "settings_restart_local", deps),
