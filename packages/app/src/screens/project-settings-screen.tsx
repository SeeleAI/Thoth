...
const WORKTREE_DOCS_URL = "https://thoth.sh/docs/worktrees";
...
function ProjectSettingsBody({
  ...
  const loadedConfig: ThothConfigRaw | null = data?.ok ? (data.config ?? {}) : null;
  const loadedRevision: ThothConfigRevision | null = data?.ok ? data.revision : null;
...
interface RenderContentInput {
  ...
  loadedConfig: ThothConfigRaw | null;
  loadedRevision: ThothConfigRevision | null;
...
function revisionToKey(revision: ThothConfigRevision | null): string {
...
interface ProjectConfigFormProps {
  baseConfig: ThothConfigRaw;
  revision: ThothConfigRevision | null;
  repoRoot: string;
  queryKey: readonly [string, string, string];
  client: DaemonClient;
  onReload: () => void;
