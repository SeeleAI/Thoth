...
import { BrowserAutomationRequestCoordinator, mountBrowserAutomationHandler } from "./handler";
...
class FakeBrowserBridge {
  ...
  public pendingResponse: Promise<BrowserAutomationResponsePayload> | null = null;
...
class BrowserAutomationHandlerHarness {
  ...
  public readonly coordinator = new BrowserAutomationRequestCoordinator();
...
function browserCloseTabRequest(
  browserId: string,
...
describe("mountBrowserAutomationHandler", () => {
  ...
  test("browser_close_tab returns not found after the tab is gone", async () => {
    ...
    browser.receive(browserCloseTabRequest(result.browserId));
    await flushAsyncWork();
    browser.receive(browserCloseTabRequest(result.browserId, "req-close-tab-2"));
    await flushAsyncWork();

    expect(browser.client.payloadAt(2)).toEqual({
      requestId: "req-close-tab-2",
      ok: false,
      error: {
        code: "browser_tab_not_found",
        message: `No browser tab found for ID: ${result.browserId}`,
  ...
  test("unimplemented preload IPC reports browser_unsupported", async () => {
    ...
    browser.browser.thrownError = new Error(
      'No handler registered for "thoth:browser:execute-automation-command"',
    );
    browser.mount();
  ...
  test("delivers one in-flight command response through the replacement daemon client", async () => {
    let resolveResponse!: (payload: BrowserAutomationResponsePayload) => void;
    const pendingResponse = new Promise<BrowserAutomationResponsePayload>((resolve) => {
      resolveResponse = resolve;
    });
    const browser = new FakeBrowserBridge();
    browser.pendingResponse = pendingResponse;
    const coordinator = new BrowserAutomationRequestCoordinator();
    const firstClient = new FakeDaemonClient();
    const secondClient = new FakeDaemonClient();
    const firstUnmount = mountBrowserAutomationHandler({
      client: firstClient,
      serverId: "server-1",
      getHost: () => ({ browser }),
      coordinator,
    ...
    const request = browserAutomationRequest();
    firstClient.receive(request);
    await flushAsyncWork();
    expect(browser.executedRequests).toEqual([request]);
    firstUnmount();

    const secondUnmount = mountBrowserAutomationHandler({
      client: secondClient,
      serverId: "server-1",
      getHost: () => ({ browser }),
      coordinator,
    });
    secondClient.receive(request);
    resolveResponse({
    ...
    expect(browser.executedRequests).toEqual([request]);
    expect(firstClient.sentResponses).toEqual([]);
    expect(secondClient.sentResponses).toEqual([
      {
        type: "browser.automation.execute.response",
        payload: {
          requestId: request.requestId,
