...
describe("git-actions-policy", () => {
  ...
  it("keeps push available for a no-upstream Thoth worktree with local commits", () => {
    const actions = buildGitActions(
      createInput({
        hasRemote: true,
  ...
  it("only shows archive worktree for thoth worktrees", () => {
    const hidden = buildGitActions(createInput());
    const shown = buildGitActions(createInput({ isThothOwnedWorktree: true }));
