...
function invalidateWorktreeList() {
  void appQueryClient.invalidateQueries({
    predicate: (query) =>
      Array.isArray(query.queryKey) && query.queryKey[0] === "thothWorktreeList",
  });
  void appQueryClient.invalidateQueries({
    predicate: (query) =>
      Array.isArray(query.queryKey) && query.queryKey[0] === "sidebarThothWorktreeList",
  });
}
  ...
  appQueryClient.setQueriesData(
    {
      predicate: (query) =>
        ...
        query.queryKey[0] === "thothWorktreeList" &&
  ...
  appQueryClient.setQueriesData(
    {
      predicate: (query) =>
        Array.isArray(query.queryKey) &&
        query.queryKey[0] === "sidebarThothWorktreeList" &&
        query.queryKey[1] === serverId,
    },
    removeFromList,
  );
...
function isWorktreeListQuery(input: { queryKey: QueryKey; serverId: string }): boolean {
  return (
    ...
    (input.queryKey[0] === "thothWorktreeList" ||
      input.queryKey[0] === "sidebarThothWorktreeList") &&
  ...
  archiveWorktree: async ({ serverId, cwd, worktreePath, workspaceId }) => {
    await runCheckoutAction({
      ...
      run: async () => {
        ...
        try {
          const payload = await client.archiveThothWorktree({
            worktreePath,
