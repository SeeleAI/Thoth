    ...
    const projects = buildSidebarProjectsFromStructure({
      projects: [
        project({
          ...
          hosts: [
            {
              serverId: "relay:thoth-host",
              iconWorkingDir: "/repo/project-1",
              canCreateWorktree: true,
            },
          ],
          workspaceKeys: ["relay:thoth-host:ws-main"],
        }),
      ],
    });

    expect(projects[0]?.workspaces[0]).toMatchObject({
      workspaceKey: "relay:thoth-host:ws-main",
      serverId: "relay:thoth-host",
      workspaceId: "ws-main",
    });
  });
...
describe("shared sidebar workspace model", () => {
  it("feeds project placement and status grouping from the same cross-host workspace identities", () => {
    const model = buildSidebarWorkspacePlacementModel({
      projects: [
        project({
          projectKey: "thoth/thoth",
          projectName: "thoth/thoth",
          iconWorkingDir: "/repo/thoth/thoth",
          hosts: [
            { serverId: "host-a", iconWorkingDir: "/repo/thoth/thoth", canCreateWorktree: true },
            { serverId: "host-b", iconWorkingDir: "/repo/thoth/thoth", canCreateWorktree: true },
          ],
    ...
    expect(model.projectNamesByKey).toEqual(new Map([["thoth/thoth", "thoth/thoth"]]));
  });
});
    ...
    const projects = buildSidebarProjectsFromStructure({
      projects: [
        project({
          projectKey: "thoth/thoth",
          hosts: [
            { serverId: "host-a", iconWorkingDir: "/repo/thoth", canCreateWorktree: true },
            { serverId: "host-b", iconWorkingDir: "/repo/thoth", canCreateWorktree: true },
          ],
          workspaceKeys: ["host-a:main", "host-b:feature"],
        }),
      ],
