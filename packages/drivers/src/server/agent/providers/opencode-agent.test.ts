...
describe("OpenCodeHarnessAdapter adapter smoke tests", () => {
  ...
  test("fetchCatalog returns models with required fields", async () => {
    ...
    expect(openCodeClient.calls.providerList).toEqual([{ directory: runtimeDir }]);
    rmSync(thothHome, { recursive: true, force: true });
  }, 60_000);

  test("fetchCatalog releases the acquired server when the runtime directory cannot be created", async () => {
    const runtime = new TestOpenCodeHarness();
    const thothHome = tmpCwd();
    const runtimeDir = path.join(thothHome, "provider-runtime");
  ...
  test("fetchCatalog releases the acquired server when the runtime directory cannot be resolved", async () => {
    ...
    const client = new OpenCodeHarnessAdapter(logger, undefined, {
      ...
      createClient: runtime.createClient,
      resolveRuntimeDir: () => {
        throw new Error("cannot resolve provider runtime directory");
      },
    });

    await expect(client.fetchCatalog({ scope: "global", force: false })).rejects.toThrow(
      "cannot resolve provider runtime directory",
    );
  ...
  test("provider run mode is unsupported without a completed native Plan item", async () => {
    const cwd = tmpCwd();
    const runtime = new TestOpenCodeHarness();
    const openCodeClient = new TestOpenCodeClient();
    openCodeClient.sessionPromptAsyncEvents = assistantTurnEvents({
  ...
  test("custom agents defined in opencode.json appear in available modes", async () => {
    ...
    openCodeClient.appAgentsResponse = {
      data: [
        {
          name: "thoth-test-custom",
          description: "Custom agent defined for Thoth integration test",
          mode: "primary",
        },
    ...
    expect(modes.map((mode) => mode.id)).toEqual(["thoth-test-custom"]);

    const custom = modes.find((mode) => mode.id === "thoth-test-custom");
    expect(custom).toBeDefined();
    expect(custom!.label).toBe("Thoth-test-custom");
    expect(custom!.description).toBe("Custom agent defined for Thoth integration test");
    ...
    const parts = __openCodeInternals.buildOpenCodePromptParts([
      {
        ...
        url: "https://github.com/thoth/thoth/issues/55",
        body: "Issue body",
      },
    ]);
  ...
  test("fails the turn when OpenCode reports MCP add failure in data payload", async () => {
    ...
    try {
      ...
      await expect(collectTurnEvents(streamSession(session, "hello"))).rejects.toThrow(
        /Failed to add OpenCode MCP server 'thoth': SSE error/,
      );
