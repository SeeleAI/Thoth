...
import { resolveOpenCodeRuntimeDir } from "./paths.js";
...
export class OpenCodeServerManager implements OpenCodeServerManagerLike {
  ...
  private readonly managedProcesses?: ManagedProviderProcessPort;
  ...
  constructor(options: OpenCodeServerManagerOptions) {
    ...
    this.resolveRuntimeDir = options.resolveRuntimeDir ?? resolveOpenCodeRuntimeDir;
  ...
  private async startServer(launchEnv?: Record<string, string>): Promise<OpenCodeServerGeneration> {
    ...
    // The helper needs a neutral runtime cwd so it never indexes the user's home.
    const serverCwd = this.resolveRuntimeDir();
