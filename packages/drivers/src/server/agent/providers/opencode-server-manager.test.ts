...
} from "@thoth/drivers/internal/server/agent/providers/opencode/server-manager";
...
describe("OpenCodeServerManager managed process ledger", () => {
  ...
  test("starts helper server from the neutral provider runtime directory", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "opencode-server-home-"));
    const runtimeDir = path.join(tempDir, "provider-runtime");
    try {
      const { manager, runtime } = createTestManager([4603], { runtimeDir });
      ...
      expect(runtime.spawnCalls).toEqual([
        expect.objectContaining({
          ...
          options: expect.objectContaining({ cwd: runtimeDir }),
        }),
      ]);
...
function createTestManager(
  ports: number[],
  options: { autoAnnounce?: boolean; runtimeDir?: string } = {},
): {
