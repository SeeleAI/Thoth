...
export function findACPPlanMode(modes: readonly AgentMode[]): AgentMode | null {
  const tokenPattern = /(^|[/:._-])plan(?:ning)?($|[/:._-])/u;
  return (
    modes.find((mode) => {
      const id = mode.id.trim().toLowerCase();
      const label = mode.label.trim().toLowerCase();
      return id === "plan" || label === "plan" || tokenPattern.test(id) || tokenPattern.test(label);
    }) ?? null
  );
}
      ...
      return await this.runACPRequest(() =>
        Promise.race([
          transport.connection.initialize({
            ...
            clientInfo: { name: "Thoth", version: "dev" },
          }),
...
export class ACPHarnessThread implements HarnessThread, ACPClient {
  ...
  private async spawnProcess(): Promise<SpawnedACPProcess> {
    ...
    // Take ownership before initialize so the outer initialization guard can close the process
    // even when the ACP handshake itself rejects.
    this.child = child;
