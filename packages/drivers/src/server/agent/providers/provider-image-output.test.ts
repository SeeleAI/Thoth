...
} from "@thoth/drivers/internal/server/agent/providers/provider-image-output";
...
describe("isProviderImageMarkdown", () => {
  test("matches the markdown emitted for a materialized attachment", () => {
    expect(isProviderImageMarkdown(`![Image](/tmp/thoth-attachments/${HASH}.png)`)).toBe(true);
    expect(isProviderImageMarkdown(`![shot](/var/folders/x/thoth-attachments/${HASH}.webp)`)).toBe(
      true,
    );
    // Windows: backslash path separators are doubled by escapeMarkdownImageSource.
    expect(
      isProviderImageMarkdown(
        `![Image](C:\\\\Users\\\\me\\\\AppData\\\\Local\\\\Temp\\\\thoth-attachments\\\\${HASH}.png)`,
      ),
    ).toBe(true);
  });
  ...
  test("emits Windows file paths as file URIs", () => {
    const markdown = renderImageMarkdown(
      `C:\\Users\\me\\AppData\\Local\\Temp\\thoth-attachments\\${HASH}.png`,
    );

    expect(markdown).toBe(
      `![Image](file:///C:/Users/me/AppData/Local/Temp/thoth-attachments/${HASH}.png)`,
    );
  ...
  test("rejects user-authored markdown that is not a materialized attachment", () => {
    ...
    expect(isProviderImageMarkdown("![diagram](./thoth-attachments/notes.png)")).toBe(false);
