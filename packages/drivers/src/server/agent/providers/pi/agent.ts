...
const THOTH_PI_CAPTURE_EXTENSION_COMMAND = "thoth_capture_entries";
...
function createPiMcpConfigFile(servers: Record<string, McpServerConfig>): PiMcpConfigFile {
  const dir = mkdtempSync(join(tmpdir(), "thoth-pi-mcp-"));
...
function createPiThothExtensionFile(): PiTempFile {
  const dir = mkdtempSync(join(tmpdir(), "thoth-pi-extension-"));
  const filePath = join(dir, "thoth-integration.mjs");
  writeFileSync(
    filePath,
    `
	...
	function emitCommandResult(ctx, requestId, result) {
	  ctx.ui.notify(
	    "${THOTH_PI_COMMAND_RESULT_MARKER} " + JSON.stringify({ requestId, ...result }),
	    result.ok ? "info" : "error",
	  );
	}

	export default function thothIntegration(pi) {
	  ...
	  pi.registerCommand("${THOTH_PI_CAPTURE_EXTENSION_COMMAND}", {
	    description: "Internal Thoth entry capture bridge",
	  ...
	  pi.registerCommand("${THOTH_PI_TREE_EXTENSION_COMMAND}", {
	    description: "Internal Thoth tree navigation bridge",
	    handler: async (args, ctx) => {
	      const payload = decodePayload(args.trim());
	      try {
...
export class PiRpcAgentSession implements AgentSession {
  ...
  private async runPiTreeExtensionCommand(targetId: string): Promise<unknown> {
    ...
    await this.runtimeSession.prompt(`/${THOTH_PI_TREE_EXTENSION_COMMAND} ${payload}`);
  ...
  private async requestEntryCapture(reason: string): Promise<void> {
    ...
    await this.runtimeSession.prompt(`/${THOTH_PI_CAPTURE_EXTENSION_COMMAND} ${payload}`);
    await resultPromise;
  }
  ...
  private handleEntryCaptureMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, THOTH_PI_ENTRY_CAPTURE_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleCommandResultMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, THOTH_PI_COMMAND_RESULT_MARKER);
    if (!payload) {
      return false;
    }
    if (typeof payload.requestId !== "string") {
  ...
  async createSession(
    ...
    try {
      runtimeSession = await this.runtime.startSession({
        ...
        extensionPaths: [thothExtension.path],
      });
    } catch (error) {
      mcpConfig?.cleanup();
      thothExtension.cleanup();
      throw error;
    }
    try {
