import type {
  ...
  PiRuntimeSession,
  PiStartSessionInput,
} from "@thoth/drivers/internal/server/agent/providers/pi/runtime";
import type {
  PiAgentMessage,
  PiModel,
  PiRpcSlashCommand,
  ...
  PiSessionState,
  PiSessionStats,
} from "@thoth/drivers/internal/server/agent/providers/pi/rpc-types";
import { buildPiLaunch } from "@thoth/drivers/internal/server/agent/providers/pi/runtime";
...
export class FakePiSession implements PiRuntimeSession {
  ...
  private handleEntryCaptureCommand(message: string): void {
    const prefix = "/thoth_capture_entries ";
    if (!message.startsWith(prefix)) {
      return;
    }
    const payload = JSON.parse(
  ...
  emitEntryCapture(requestId?: string, reason = "test"): void {
    this.emit({
      ...
      method: "notify",
      message: `THOTH_ENTRY_CAPTURE ${JSON.stringify({
        reason,
        requestId,
  ...
  private emitExtensionCommandResult(
    ...
    this.emit({
      ...
      method: "notify",
      message: `THOTH_COMMAND_RESULT ${JSON.stringify({ requestId, ...result })}`,
    });
  }
}
