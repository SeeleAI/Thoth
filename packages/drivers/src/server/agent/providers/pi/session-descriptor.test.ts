...
} from "@thoth/drivers/internal/server/agent/providers/pi/session-descriptor";
...
test("Pi import config preserves the latest recorded model and thinking level", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "thoth-pi-session-model-"));
...
test("Pi import config can infer model from assistant messages", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "thoth-pi-session-message-model-"));
  const cwd = path.join(root, "repo");
  const sessionFile = await writeSession(root, [
    {
      type: "session",
...
test("Pi import config preserves thinking before a later model in large sessions", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "thoth-pi-session-large-thinking-"));
  const cwd = path.join(root, "repo");
