...
import { PiCliRuntime } from "@thoth/drivers/internal/server/agent/providers/pi/cli-runtime";
import type { PiCommandsRpcType } from "@thoth/drivers/internal/server/agent/providers/pi/rpc-types";
import type { PiRuntimeLaunch } from "@thoth/drivers/internal/server/agent/providers/pi/runtime";
...
describe("PiCliRuntime", () => {
  ...
  test("passes an MCP config path to Pi", async () => {
    ...
    await runtime.startSession({
      cwd: "/workspace/project",
      mcpConfigPath: "/tmp/thoth-pi-mcp/mcp.json",
    });
    ...
    expect(launches).toEqual([
      expect.objectContaining({
        cwd: "/workspace/project",
        mcpConfigPath: "/tmp/thoth-pi-mcp/mcp.json",
        argv: ["pi", "--mode", "rpc", "--mcp-config", "/tmp/thoth-pi-mcp/mcp.json"],
      }),
    ]);
  });
