...
import {
  ...
  parseToolResult,
  resolveToolCallName,
} from "@thoth/drivers/internal/server/agent/providers/pi/tool-call-mapper";
...
describe("Pi tool call mapper", () => {
  ...
  test("normalizes Pi MCP proxy calls from requested tool args while running", () => {
    const toolCall = parseToolArgs("mcp", {
      tool: "thoth_list_models",
      args: '{"provider":"pi"}',
    });

    expect(resolveToolCallName(toolCall, null)).toBe("thoth.list_models");
  });
  ...
  test("normalizes Pi MCP proxy calls from result details when completed", () => {
    ...
    expect(resolveToolCallName(toolCall, result)).toBe("thoth.list_models");
  });
});
