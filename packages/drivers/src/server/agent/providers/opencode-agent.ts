...
import { resolveOpenCodeRuntimeDir } from "./opencode/paths.js";
import {
...
export class OpenCodeAgentClient implements AgentClient {
  ...
  private readonly resolveRuntimeDir: () => string;
  ...
  constructor(
    ...
    this.resolveRuntimeDir = deps.resolveRuntimeDir ?? resolveOpenCodeRuntimeDir;
  }
  ...
  async fetchCatalog(options: FetchCatalogOptions): Promise<ProviderCatalog> {
    ...
    try {
      // OpenCode treats the catalog directory as a workspace. Use the neutral
      // runtime directory so global discovery never indexes the user's home.
      const directory = isGlobalCatalog ? this.resolveRuntimeDir() : options.cwd;

      if (isGlobalCatalog) {
        await fs.mkdir(directory, { recursive: true });
        this.logger.debug(
          { directory },
          "opencode catalog refresh: using provider runtime directory",
        );
      }
