...
import {
  ...
  revertClaudeFiles,
} from "@thoth/drivers/internal/server/agent/providers/claude/rewind";
import { FakeClaudeSdk } from "@thoth/drivers/internal/server/agent/providers/claude/test-rewind-claude-sdk";
...
describe("Claude rewind", () => {
  ...
  test("translates Thoth timeline message ids before forking", async () => {
    const claude = new FakeClaudeSdk();
    let sessionId = "original-session";
  ...
  test("translates Thoth timeline message ids before rewinding files", async () => {
    const claude = new FakeClaudeSdk();
