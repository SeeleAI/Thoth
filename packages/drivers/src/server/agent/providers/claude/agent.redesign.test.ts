...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "THOTH_ENV_SENTINEL_SECRET";
  const runtimeSecret = "THOTH_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "THOTH_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.THOTH_TEST_SENTINEL_SECRET;
  process.env.THOTH_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        THOTH_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.THOTH_TEST_SENTINEL_SECRET;
    } else {
      process.env.THOTH_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
...
test("Grep tool_result string content flows to a search detail with content", async () => {
  ...
  try {
    ...
    const { mapClaudeCompletedToolCall } =
      await import("@thoth/drivers/internal/server/agent/providers/claude/tool-call-mapper");
