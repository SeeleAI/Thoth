...
import { CodexAppServerClient } from "@thoth/drivers/internal/server/agent/providers/codex/app-server-transport";
import {
    ...
    providerOverrides: {
      [providerId]: {
        ...
        env: {
          ...
          THOTH_FAKE_CODEX_CAPTURE: capturedRequestsPath,
        },
      },
    },
  });
...
describe("Codex app-server provider", () => {
  ...
  test("reattaches Thoth semantic dynamic tools when resuming a persisted thread", async () => {
    ...
    const fakeClient: CodexClientLike = {
      async request(method: string, params?: unknown) {
        ...
        if (method === "thread/resume") return { thread: { id: "persisted-tool-thread" } };
        return null;
      },
    };
    const session = new CodexAppServerAgentSession(
  ...
  test("initializes Codex app-server without making Thoth the request originator", async () => {
  ...
  test("resolves Codex skill slash commands into app-server skill input", async () => {
          ...
          data: [
            {
              ...
              skills: [
                {
                  name: "thoth-implement",
                  description: "Execute an existing Thoth plan.",
                  path: "/tmp/skills/thoth-implement/SKILL.md",
                },
              ],
              errors: [],
            },
    ...
    await session.startTurn("/thoth-implement in a worktree, remember to use Claude for the UI");
    ...
    expect(turnStartCall?.[1]).toEqual(
      expect.objectContaining({
        input: [
          ...
          {
            type: "text",
            text: "$thoth-implement in a worktree, remember to use Claude for the UI",
            text_elements: [],
          },
        ],
      }),
    ...
    const commands = await listCommandsFromFakeCodex([
      {
        ...
        path: "/Users/test/.agents/skills/thoth/SKILL.md",
      },
      {
        name: "thoth",
        description: "Shared orchestration skill.",
        path: "/Users/test/.codex/skills/thoth/SKILL.md",
      },
    ]);

    expect(commands.filter((command) => command.name === "thoth")).toEqual([
      {
        name: "thoth",
        description: "Shared orchestration skill.",
        argumentHint: "",
    ...
    const input = await codexAppServerTurnInputFromPrompt(
      [
        { type: "text", text: "Please review this" },
        {
          ...
          url: "https://github.com/thoth/thoth/issues/456",
        },
      ],
      logger,
    );
  ...
  test("builds app-server env from launch-context env overrides", () => {
    const launchContext: AgentLaunchContext = {
      env: {
        THOTH_AGENT_ID: "00000000-0000-4000-8000-000000000301",
        THOTH_TEST_FLAG: "codex-launch-value",
      },
    };
    const env = buildCodexAppServerEnv(
      {
        env: {
          THOTH_AGENT_ID: "runtime-value",
          THOTH_TEST_FLAG: "runtime-test-value",
        },
      },
      launchContext.env,
    );

    expect(env.THOTH_AGENT_ID).toBe(launchContext.env?.THOTH_AGENT_ID);
    ...
    expect(events).toEqual([
      {
        ...
        item: {
          type: "assistant_message",
          text: "![Image](/tmp/thoth image.png)",
        },
      },
    ]);
  });
  ...
  test("materializes imageGeneration base64 results before rendering markdown", () => {
    ...
    expect(source).toMatch(/thoth-attachments[\\/].+\.png$/);
