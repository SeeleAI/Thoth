...
function signalTreeOrChild(child: TreeKillTarget, signal: NodeJS.Signals): Promise<void> {
  ...
  if (process.platform !== "win32") {
    try {
      // Provider processes are launched as their own POSIX process groups. A
      // negative pid terminates the group without requiring `ps` or another
      // host utility. Non-detached children fall back to the direct handle.
