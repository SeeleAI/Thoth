...
import type { TerminalCell, TerminalState } from "@thoth/protocol/messages";
import { TerminalInputModeTracker } from "@thoth/protocol/terminal-input-mode";
import { TerminalActivityTracker } from "./activity/terminal-activity-tracker.js";
import type { TerminalActivity, TerminalActivityState } from "@thoth/protocol/terminal-activity";

const { Terminal } = xterm;
const require = createRequire(import.meta.url);
const THOTH_CLI_BIN_ENTRY = "@thoth/cli/bin/thoth";
...
interface BuildTerminalEnvironmentInput {
  ...
  thothCliBinDir?: string | null;
  thothHookCliPath?: string | null;
}
...
export function resolveThothCliExecutablePath(): string | null {
  ...
  if (npmBinDir) {
    const shim = resolveThothCliShim(npmBinDir);
    if (shim) {
      return shim;
    }
  }
...
function resolveThothCliBinEntrypoint(): string | null {
  try {
    return require.resolve(THOTH_CLI_BIN_ENTRY);
  } catch {
    return null;
  }
}
...
function hasThothCliShim(binDir: string): boolean {
  return resolveThothCliShim(binDir) !== null;
}

function resolveThothCliShim(binDir: string): string | null {
  for (const name of thothCliShimNames()) {
...
function thothCliShimNames(): string[] {
  return process.platform === "win32" ? ["thoth.cmd", "thoth.exe", "thoth"] : ["thoth"];
}
...
function resolveZshShellIntegrationRuntimeDir(): string {
  ...
  return join(tmpdir(), `${username}-thoth-zsh`);
}
...
function prepareZshShellIntegrationRuntimeDir(sourceDir = resolveZshShellIntegrationDir()): string {
  ...
  writePrivateFileAtomicSync(
    join(runtimeDir, "thoth-integration.zsh"),
    readFileSync(join(readableSourceDir, "thoth-integration.zsh")),
  );
  return runtimeDir;
}
...
export function buildTerminalEnvironment(
  ...
  const envWithAgentHooks = prependThothCliToPath(
    baseEnv,
    input.thothCliBinDir === undefined ? resolveThothCliBinDir() : input.thothCliBinDir,
  );
  const envWithHookCli = injectThothHookCli(
    envWithAgentHooks,
    input.thothHookCliPath === undefined ? resolveThothCliExecutablePath() : input.thothHookCliPath,
  );
...
function injectThothHookCli(
  ...
  return {
    ...env,
    THOTH_HOOK_CLI: resolvePath(resolveExternalProcessPath(cliPath)),
  };
}

function prependThothCliToPath(
  env: Record<string, string>,
  ...
  const ptyProcess = pty.spawn(spawnCommand, spawnArgs, {
    ...
    env: buildTerminalEnvironment({
      ...
      env: {
        ...env,
        ...activityEnv,
        THOTH_WORKSPACE_ID: workspaceId,
      },
    }),
  });
