import { z } from "zod";
import { BrowserAutomationBrowserIdSchema } from "@thoth/protocol/browser-automation/rpc-schemas";
...
export interface RegisterBrowserToolsOptions {
  ...
  toolGateway: Pick<ToolGateway, "authorizeScopedCapability">;
}

const HTTP_URL_ONLY_MESSAGE = "URL must use http/https only";
const WORKSPACE_CONTEXT_MESSAGE =
  "This browser tool needs a workspace. Start the agent from a Thoth Workspace before calling browser_new_tab or browser_list_tabs.";
...
export function registerBrowserTools(options: RegisterBrowserToolsOptions): void {
  options.registerTool(
    "browser_list_tabs",
    {
      ...
      description:
        "List open Thoth browser tabs for this agent's workspace across connected browser automation hosts. Use returned browserId values with tab-scoped tools.",
      inputSchema: {},
    },
    async (_input, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
  ...
  options.registerTool(
    "browser_new_tab",
    {
      ...
      description:
        "Create a new Thoth browser tab in this agent's workspace on the most recently connected browser automation host, opened in the background without switching the user's view. Pass an http(s) URL or a scheme-less host URL, which is treated as http; the returned browserId is used by tab-scoped tools.",
      inputSchema: {
        url: BrowserHttpUrlInputSchema.optional(),
      },
    },
    async ({ url }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const missingWorkspace = requireWorkspaceContext(context);
      if (missingWorkspace) {
        return missingWorkspace;
  ...
  options.registerTool(
    "browser_snapshot",
    {
      ...
      description:
        "Return a model-readable snapshot of a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs; refs come from the latest browser_snapshot of the same tab and expire when the page changes.",
      inputSchema: {
        browserId: BrowserAutomationBrowserIdSchema,
      },
    },
    async ({ browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_click",
    {
      ...
      description:
        "Click an element in a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs; refs come from the latest browser_snapshot of the same tab and expire when the page changes.",
      inputSchema: {
    ...
    async ({ ref, browserId, button, doubleClick, modifiers }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_fill",
    {
      ...
      description:
        "Fill an input-like element in a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs; refs come from the latest browser_snapshot of the same tab and expire when the page changes.",
      inputSchema: {
        ref: BrowserRefInputSchema,
        value: z.string(),
        browserId: BrowserAutomationBrowserIdSchema,
    ...
    async ({ ref, value, browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_wait",
    {
      ...
      description:
        "Wait until a Thoth browser tab contains text or reaches a URL fragment. Use browserId from browser_new_tab or browser_list_tabs; waits up to 5s by default on the browser host.",
      inputSchema: BrowserWaitInputSchema,
    },
    async ({ text, url, timeoutMs, browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    ...
    async ({ text, ref, browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    ...
    async ({ key, ref, browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_navigate",
    {
      ...
      description:
        "Navigate a Thoth browser tab to a URL. Use browserId from browser_new_tab or browser_list_tabs; pass an http(s) URL or a scheme-less host URL, which is treated as http.",
      inputSchema: { url: BrowserHttpUrlInputSchema, browserId: BrowserAutomationBrowserIdSchema },
    },
    async ({ url, browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  for (const toolConfig of [
    {
      ...
      title: "Browser back",
      description:
        "Go back in a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs.",
    },
    {
      ...
      description:
        "Go forward in a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs.",
    },
    {
      name: "browser_reload",
      command: "reload",
      ...
      description:
        "Reload a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs.",
    },
  ] as const) {
    options.registerTool(
      toolConfig.name,
  ...
  options.registerTool(
    "browser_screenshot",
    {
      ...
      description:
        "Capture a PNG screenshot of a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs. Set fullPage to true to capture the full page.",
      inputSchema: {
        browserId: BrowserAutomationBrowserIdSchema,
        fullPage: z.boolean().default(false),
      },
    },
    async ({ browserId, fullPage }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_upload",
    {
      ...
      description:
        "Set workspace files on a file input in a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs; refs come from the latest browser_snapshot of the same tab and expire when the page changes.",
      inputSchema: {
        ref: BrowserRefInputSchema,
    ...
    async ({ ref, filePaths, browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  for (const toolConfig of [
    {
      ...
      description:
        "Hover an element in a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs; refs come from the latest browser_snapshot of the same tab and expire when the page changes.",
    },
  ] as const) {
    options.registerTool(
      toolConfig.name,
      ...
      async ({ ref, browserId }, toolContext) => {
        const context = resolveBrowserToolContext(options, toolContext);
        const payload = await options.broker.execute({
          ...context,
  ...
  options.registerTool(
    "browser_select",
    {
      ...
      description:
        "Set a select element in a Thoth browser tab to a value. Use browserId from browser_new_tab or browser_list_tabs; refs come from the latest browser_snapshot of the same tab and expire when the page changes.",
      inputSchema: {
        ref: BrowserRefInputSchema,
        value: z.string(),
        browserId: BrowserAutomationBrowserIdSchema,
  ...
  options.registerTool(
    "browser_drag",
    {
      ...
      description:
        "Drag one element onto another in a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs; refs come from the latest browser_snapshot of the same tab and expire when the page changes.",
      inputSchema: {
    ...
    async ({ sourceRef, targetRef, browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_logs",
    {
      ...
      description:
        "Read recent console messages and browser performance network entries for a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs; maxEntries defaults to 50.",
      inputSchema: {
        maxEntries: z.number().int().positive().max(200).optional(),
        browserId: BrowserAutomationBrowserIdSchema,
      },
    },
    async ({ maxEntries, browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_evaluate",
    {
      ...
      description:
        "Evaluate a JavaScript function in a Thoth browser tab. Use browserId from browser_new_tab or browser_list_tabs; when ref is provided, refs come from the latest browser_snapshot and the resolved element is passed as the first argument.",
      inputSchema: {
    ...
    async ({ function: functionSource, ref, browserId }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_scroll",
    {
      ...
      description:
        "Scroll a Thoth browser tab by deltaX/deltaY CSS pixels. Use browserId from browser_new_tab or browser_list_tabs; optional ref comes from the latest browser_snapshot and centers the wheel input over that element.",
      inputSchema: {
        browserId: BrowserAutomationBrowserIdSchema,
        ref: BrowserRefInputSchema.optional(),
        deltaX: z.number(),
    ...
    async ({ browserId, ref, deltaX, deltaY }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_resize",
    {
      ...
      description:
        "Resize a Thoth browser tab's resident webview viewport. Use browserId from browser_new_tab or browser_list_tabs.",
      inputSchema: {
        browserId: BrowserAutomationBrowserIdSchema,
    ...
    async ({ browserId, width, height }, toolContext) => {
      const context = resolveBrowserToolContext(options, toolContext);
      const payload = await options.broker.execute({
        ...context,
  ...
  options.registerTool(
    "browser_close_tab",
    {
      ...
      description:
        "Close a Thoth browser tab, remove its resident webview, and unregister it from the browser automation host. Use browserId from browser_new_tab or browser_list_tabs.",
      inputSchema: {
        browserId: BrowserAutomationBrowserIdSchema,
      },
    },
...
function resolveBrowserToolContext(
  options: RegisterBrowserToolsOptions,
  toolContext: ThothToolExecutionContext,
): {
  agentId: string;
  cwd: string;
  workspaceId: string;
  ...
  const authority = options.toolGateway.authorizeScopedCapability({
    agentId: options.callerAgentId,
    context: toolContext,
  });
  const callerAgent = options.resolveCallerAgent();
  if (!callerAgent?.workspaceId) {
    throw new Error(WORKSPACE_CONTEXT_MESSAGE);
  }
  if (
    callerAgent.id !== options.callerAgentId ||
    callerAgent.workspaceId !== authority.workspaceId
  ) {
    throw new Error("The active provider execution no longer belongs to this Workspace");
  }
  return {
    ...authority,
    cwd: callerAgent.cwd,
...
function browserToolImageContent(
  result: Extract<BrowserToolsResponsePayload, { ok: true }>["result"],
): ThothToolResult["content"][number] | null {
  if (result.command !== "screenshot") {
    return null;
  }
...
function summarizeBrowserSuccess(
  ...
  if (payload.result.command === "list_tabs") {
    ...
    if (count === 0) {
      return "No Thoth browser tabs are open. Call browser_new_tab to create one.";
    }
    ...
    return withDialogs(
      [
        `Found ${count} Thoth browser tab${count === 1 ? "" : "s"}. Use these browserId values for tab-scoped browser tools.`,
        ...tabLines,
      ].join("\n"),
    );
  }
