  ...
  try {
    ...
    expect(result).toEqual({
      ...
      file: {
        ...
        mimeType: "text/plain",
        size: 11,
        path: path.join(daemon.thothHome, "uploads", "upload_req-upload-e2e", "notes.txt"),
      },
      error: null,
    });
...
test("returns home-scoped directory suggestions", async () => {
  const insideHomeDir = mkdtempSync(path.join(homedir(), "thoth-dir-suggestion-"));
  const outsideHomeDir = mkdtempSync(path.join(tmpdir(), "thoth-dir-suggestion-outside-"));
