...
function createArchiveDeps(input: ArchiveDepsInput): ArchiveTestDependencies {
  ...
  return {
    ...
    thothWorktreesBaseRoot: input.thothWorktreesBaseRoot,
...
describe("archiveByScope", () => {
  test("workspace scope archives the record and removes the directory on last reference", async () => {
    ...
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "last-ref-workspace");
  ...
  test("workspace scope keeps the directory when a sibling workspace still references it", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const thothHome = path.join(tempDir, ".thoth");
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "sibling-workspace");
  ...
  test("worktree scope archives every workspace on the directory and removes it", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const thothHome = path.join(tempDir, ".thoth");
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "worktree-scope");
  ...
  test("workspace scope never removes a non-Thoth-owned directory", async () => {
  ...
  test("worktree scope keeps the directory when one record teardown fails", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const thothHome = path.join(tempDir, ".thoth");
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "partial-failure");
  ...
  test("worktree scope removes an owned directory with zero matching records", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const thothHome = path.join(tempDir, ".thoth");
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "zero-records");
  ...
  test("marks archiving, emits an upsert carrying the archiving state, then clears it and emits a remove", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const thothHome = path.join(tempDir, ".thoth");
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "lifecycle");
  ...
  test("archives stored snapshots only for the target workspace", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const thothHome = path.join(tempDir, ".thoth");
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "snapshot-scope");
  ...
  test("worktree scope archives three workspaces on the directory and removes it", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const thothHome = path.join(tempDir, ".thoth");
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "worktree-scope-n3");
