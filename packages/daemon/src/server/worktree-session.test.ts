...
import {
  ...
  createThothWorktreeWorkflow,
  handleThothWorktreeArchiveRequest,
  handleThothWorktreeListRequest,
  resolveGitCreateBaseBranch,
  runWorktreeSetupInBackground,
  handleCreateThothWorktreeRequest,
...
import {
  createThothWorktree as createThothWorktreeService,
  type CreateThothWorktreeFn,
...
function createWorkflowForRequestTest(options: {
  thothHome: string;
  createThothWorktree?: CreateThothWorktreeFn;
  ...
  return async (input: Parameters<CreateThothWorktreeFn>[0]) => {
    const createThothWorktree =
      options.createThothWorktree ?? createThothWorktreeForTest({ thothHome: options.thothHome });
    return createThothWorktreeWorkflow(
      {
        thothHome: options.thothHome,
        createThothWorktree,
...
function createThothWorktreeForTest(options: {
  thothHome: string;
  events?: string[];
}): CreateThothWorktreeFn {
  ...
  return (input, serviceOptions) => {
    return createThothWorktreeService(input, {
      github: createGitHubServiceStub(),
...
describe("handleThothWorktreeListRequest", () => {
  test("lists worktrees through the workspace git service", async () => {
    const emitted: SessionOutboundMessage[] = [];
    const workspaceGitService = {
      listWorktrees: vi.fn().mockResolvedValue([
        {
          path: "/tmp/thoth-home/worktrees/repo/feature",
    ...
    await handleThothWorktreeListRequest(
      {
    ...
    expect(emitted).toContainEqual({
      type: "thoth_worktree_list_response",
      payload: {
        worktrees: [
          {
            worktreePath: "/tmp/thoth-home/worktrees/repo/feature",
            createdAt: "2026-04-12T00:00:00.000Z",
            branchName: "feature",
            head: "abc123",
          },
...
describe("create-agent worktree setup boundary", () => {
  test("agent setup continuation starts setup for the created agent timeline", async () => {
    ...
    try {
      const result = await createThothWorktreeWorkflow(
        {
          thothHome,
          createThothWorktree: createThothWorktreeForTest({ thothHome }),
...
function createGitRepo(options?: { thothConfig?: Record<string, unknown> }) {
  ...
  if (options?.thothConfig) {
    writeFileSync(path.join(repoDir, "thoth.json"), JSON.stringify(options.thothConfig, null, 2));
  }
...
describe("runWorktreeSetupInBackground", () => {
  ...
  test("archives the pending workspace and emits a failed snapshot when setup cannot start", async () => {
    ...
    writeFileSync(path.join(repoDir, "thoth.json"), "{ invalid json\n");
    execFileSync("git", ["add", "thoth.json"], { cwd: repoDir, stdio: "pipe" });
    ...
    expect(progressMessages[1]?.payload.error).toMatch(
      /Failed to parse thoth\.json at .*thoth\.json/,
    );
    expect(progressMessages[1]?.payload.detail.commands).toEqual([]);
    expect(snapshots.get(workspaceId)).toMatchObject({
      status: "failed",
      error: expect.stringMatching(/Failed to parse thoth\.json at .*thoth\.json/),
    });
    ...
    const snapshots = new Map([
      [
        "ws-feature-a",
        {
          ...
          detail: {
            type: "worktree_setup",
            worktreePath: "/repo/.thoth/worktrees/feature-a",
...
describe("handleCreateThothWorktreeRequest", () => {
  const cleanupPaths: string[] = [];
  ...
  test("checks out the GitHub PR branch when githubPrNumber is supplied", async () => {
    ...
    await handleCreateThothWorktreeRequest(
      {
        thothHome,
        ...
        emit: (message) => emitted.push(message),
        sessionLogger: logger,
        createThothWorktreeWorkflow: createWorkflowForRequestTest({ thothHome }),
      },
      {
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "create_thoth_worktree_response" }> =>
        message.type === "create_thoth_worktree_response",
    );
  ...
  test("buildAgentSessionConfig passes prompt and attachment context into worktree creation", async () => {
    const createThothWorktree = vi.fn(async () => ({
      worktree: {
    ...
    expect(createThothWorktree).toHaveBeenCalledWith(
  ...
  test("createThothWorktreeForTest forwards the default branch resolver for branch-off intents", async () => {
    const { tempDir, repoDir } = createGitRepo();
    cleanupPaths.push(tempDir);
    const thothHome = path.join(tempDir, ".thoth");
    const resolveDefaultBranch = vi.fn(async () => "main");

    const result = await createThothWorktreeForTest({ thothHome })(
      {
        cwd: repoDir,
      ...
      await handleCreateThothWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createThothWorktreeWorkflow: createWorkflowForRequestTest({
            thothHome,
            createThothWorktree: createThothWorktreeForTest({ thothHome, events }),
          }),
  ...
  test("creates the worktree before emitting the response", async () => {
    ...
    try {
      await handleCreateThothWorktreeRequest(
        {
          ...
          createThothWorktreeWorkflow: createWorkflowForRequestTest({
            thothHome,
            createThothWorktree: async (input) => {
              const result = await createThothWorktreeForTest({ thothHome })(input);
      ...
      expect(emitted.map((message) => message.type).slice(0, 2)).toEqual([
        "create_thoth_worktree_response",
        "workspace_update",
      ]);
...
describe("handleThothWorktreeArchiveRequest worktree scope", () => {
  const cleanupPaths: string[] = [];
  ...
  test("archives every active workspace on the directory and removes it", async () => {
    ...
    await handleThothWorktreeArchiveRequest(
      {
        thothHome,
        github: createGitHubServiceStub(),
        workspaceGitService: {
    ...
    expect(
      emitted.find((message) => message.type === "thoth_worktree_archive_response"),
    ).toMatchObject({
      payload: {
        success: true,
        error: null,
  ...
  test("ignores deleteWorktreeFromDisk:true and derives directory removal from remaining references", async () => {
    ...
    await handleThothWorktreeArchiveRequest(deps, {
      type: "thoth_worktree_archive_request",
    ...
    expect(existsSync(sharedCwd)).toBe(false);
    expect(
      emitted.filter((message) => message.type === "thoth_worktree_archive_response"),
    ).toHaveLength(2);
  });
});
