...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through Thoth using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "thoth-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: Thoth issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
