...
describe("agent MCP end-to-end (offline)", () => {
  test("create_agent runs initial prompt and affects filesystem", async () => {
    const thothHome = await mkdtemp(path.join(os.tmpdir(), "thoth-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "thoth-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "thoth-agent-cwd-"));
    const port = await getAvailablePort();

    const daemonConfig: ThothDaemonConfig = {
    ...
    const daemon = await createThothDaemon(daemonConfig, pino({ level: "silent" }));
    await daemon.start();
  ...
  test("create_agent auto-injects thoth MCP by default and can be disabled", async () => {
    const thothHome = await mkdtemp(path.join(os.tmpdir(), "thoth-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "thoth-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "thoth-agent-cwd-"));
    const port = await getAvailablePort();
    ...
    const disabledThothHome = await mkdtemp(path.join(os.tmpdir(), "thoth-home-disabled-"));
    const disabledStaticDir = await mkdtemp(path.join(os.tmpdir(), "thoth-static-disabled-"));
    const disabledAgentCwd = await mkdtemp(path.join(os.tmpdir(), "thoth-agent-cwd-disabled-"));
    const disabledPort = await getAvailablePort();
    const disabledRecorder: LaunchRecorder = { recordedLaunches: [] };
    const disabledDaemonConfig: ThothDaemonConfig = {
      listen: `127.0.0.1:${disabledPort}`,
      thothHome: disabledThothHome,
      corsAllowedOrigins: [],
      hostnames: true,
      mcpEnabled: true,
    ...
    try {
      ...
      await rm(disabledThothHome, { recursive: true, force: true });
  ...
  test("create_agent with worktree is async and boots terminals only after setup success", async () => {
    const thothHome = await mkdtemp(path.join(os.tmpdir(), "thoth-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "thoth-static-"));
    const repoRoot = await mkdtemp(path.join(os.tmpdir(), "thoth-worktree-repo-"));
    const port = await getAvailablePort();
