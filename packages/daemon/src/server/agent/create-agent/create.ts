...
import type {
  ...
  CreateThothWorktreeSetupContinuationInput,
...
async function resolveMcpCwd(params: {
  ...
  const createdWorktree = await createMcpWorktree({
    ...
    createThothWorktree: dependencies.createThothWorktree,
...
interface CreateMcpWorktreeOptions {
  ...
  createThothWorktree: CreateThothWorktreeWorkflowFn | undefined;
...
async function createMcpWorktree(
  ...
  try {
    if (!options.createThothWorktree) {
      throw new Error("Thoth worktree service is not configured");
    }
    return await options.createThothWorktree(options.input, {
