...
function configureOpenCodeProviderStub(
  ...
  const opencodeModes: AgentMode[] = [
    ...
    { id: "thoth-custom", label: "Thoth Custom", description: "Custom OpenCode agent" },
  ];
...
function createThothWorktreeForMcpTest(options: {
  thothHome: string;
  broadcasts: string[];
  ...
  return async (input, serviceOptions) => {
    ...
    const result = await createThothWorktreeService(input, {
      github,
...
describe("create_agent MCP tool", () => {
  ...
  it("registers and broadcasts a workspace when create_agent creates a worktree", async () => {
    ...
    try {
      ...
      const server = await createAgentMcpServer({
        ...
        providerSnapshotManager: createOpenCodeManager().manager,
        thothHome,
        createThothWorktree: createThothWorktreeForMcpTest({
          thothHome,
          broadcasts,
          createdWorkspaceIds,
          setupContinuations,
  ...
  it("creates a create_agent branch-off worktree without invoking the legacy metadata branch rename", async () => {
    ...
    try {
      ...
      const server = await createAgentMcpServer({
        ...
        providerSnapshotManager: createOpenCodeManager().manager,
        thothHome,
        createThothWorktree: createThothWorktreeForMcpTest({ thothHome, broadcasts }),
  ...
  it("passes create_agent GitHub PR worktrees through workspace creation without metadata branch rename", async () => {
    ...
    const createThothWorktree = vi.fn(
      async (
        ...
        options?: Parameters<CreateThothWorktreeWorkflowFn>[1],
      ) => ({
        worktree: {
          branchName: "pr-123",
  ...
  it("routes list_worktrees through WorkspaceGitService", async () => {
    ...
    const workspaceGitService = {
      ...
      listWorktrees: vi.fn(async () => [
        {
          path: "/tmp/thoth/worktrees/repo/feature",
          branchName: "feature",
  ...
  it("allows caller agents to override cwd and applies caller context labels", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const baseDir = await mkdtemp(join(tmpdir(), "thoth-mcp-test-"));
