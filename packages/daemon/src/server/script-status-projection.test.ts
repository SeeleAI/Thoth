...
import { WorkspaceScriptPayloadSchema } from "@thoth/protocol/messages";
import type { ScriptHealthState } from "./script-health-monitor.js";
import { WorkspaceScriptRuntimeStore } from "./workspace-script-runtime-store.js";
import { readThothConfig } from "../utils/worktree.js";
import type { ThothConfig } from "@thoth/protocol/thoth-config-schema";
import { createTestLogger } from "../test-utils/test-logger.js";

function createWorkspaceRepo(options?: {
  branchName?: string;
  thothConfig?: Record<string, unknown>;
...
function buildPayloads(input: {
  ...
  const thothConfig =
    input.thothConfig !== undefined ? input.thothConfig : loadConfig(input.workspaceDirectory);
...
function loadConfig(repoRoot: string): ThothConfig | null {
...
describe("script-status-projection", () => {
  ...
  it("readThothConfig fails with configPath and error when thoth.json is malformed", () => {
    const workspace = createWorkspaceRepo();
    const configPath = path.join(workspace.repoDir, "thoth.json");
    writeFileSync(
      configPath,
      '{\n<<<<<<< HEAD\n  "scripts": {}\n=======\n  "scripts": {}\n>>>>>>> origin/main\n}\n',
    );
    ...
    try {
      const result = readThothConfig(workspace.repoDir);
  ...
  it("buildWorkspaceScriptPayloads given thothConfig=null still surfaces orphaned runtime scripts", () => {
