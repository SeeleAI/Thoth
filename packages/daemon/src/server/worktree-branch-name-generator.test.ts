...
describe("generateBranchNameFromFirstAgentContext", () => {
  ...
  test("keeps the branch slug validator fallback when instructions are present", async () => {
    const repoRoot = createTempDir("thoth-branch-config-");
    const worktreeRoot = createTempDir("thoth-branch-worktree-");
    mkdirSync(path.join(worktreeRoot, ".git"));
    writeThothWorktreeMetadata(worktreeRoot, { baseRefName: "main" });
    writeThothWorktreeFirstAgentBranchAutoNameMetadata(worktreeRoot, {
...
async function generateBranchPromptWithConfig(config: unknown): Promise<{ prompt: string }> {
  ...
  if (typeof config === "string") {
    writeFileSync(path.join(repoRoot, "thoth.json"), config);
...
function writeConfig(repoRoot: string, config: unknown): void {
  writeFileSync(path.join(repoRoot, "thoth.json"), `${JSON.stringify(config)}\n`);
}
