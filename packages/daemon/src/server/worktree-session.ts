...
interface CreateThothWorktreeInBackgroundDependencies {
  thothHome?: string;
  worktreesRoot?: string;
...
interface CreateThothWorktreeWorkflowDependencies extends CreateThothWorktreeInBackgroundDependencies {
  createThothWorktree: (
    input: CreateThothWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
...
export type CreateThothWorktreeSetupContinuationInput =
  | { kind: "workspace" }
...
export type CreateThothWorktreeWorkflowResult = CreateThothWorktreeResult & {
  setupContinuation?: AgentWorktreeSetupContinuation;
};

export type CreateThothWorktreeWorkflowFn = (
  input: CreateThothWorktreeInput,
  options?: {
    resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    setupContinuation?: CreateThothWorktreeSetupContinuationInput;
...
interface HandleCreateThothWorktreeRequestDependencies {
  thothHome?: string;
  worktreesRoot?: string;
...
export async function buildAgentSessionConfig(
  ...
  if (normalized.createWorktree) {
    ...
    const createdWorktree = await dependencies.createThothWorktree(
      {
        cwd,
...
export async function handleThothWorktreeListRequest(
  dependencies: {
    emit: EmitSessionMessage;
    thothHome?: string;
    workspaceGitService: WorkspaceGitService;
...
export async function handleThothWorktreeArchiveRequest(
  dependencies: Omit<
    ArchiveDependencies,
  ...
  try {
    ...
    if (!result.ok) {
      dependencies.emit({
        type: "thoth_worktree_archive_response",
        payload: {
          success: false,
...
export async function handleCreateThothWorktreeRequest(
  dependencies: HandleCreateThothWorktreeRequestDependencies,
  request: Extract<SessionInboundMessage, { type: "create_thoth_worktree_request" }>,
): Promise<void> {
  try {
    const commandResult = await createThothWorktreeCommand(
      {
        thothHome: dependencies.thothHome,
        worktreesRoot: dependencies.worktreesRoot,
        createThothWorktreeWorkflow: dependencies.createThothWorktreeWorkflow,
      },
      {
        cwd: request.cwd,
...
export async function createThothWorktreeWorkflow(
  dependencies: CreateThothWorktreeWorkflowDependencies,
  input: CreateThothWorktreeInput,
  options?: {
    resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    setupContinuation?: CreateThothWorktreeSetupContinuationInput;
...
export async function runWorktreeSetupInBackground(
  dependencies: CreateThothWorktreeInBackgroundDependencies,
  options: {
    requestCwd: string;
    repoRoot: string;
    workspaceId: string;
