...
async function launchOpenCodeRewindSession(
  ...
  execFileSync("git", ["config", "user.email", "thoth-test@example.com"], {
    cwd,
    stdio: "ignore",
  });
  execFileSync("git", ["config", "user.name", "Thoth Test"], { cwd, stdio: "ignore" });
...
function editPrompt(input: {
  ...
  return [
    `THOTH_OPENCODE_REWIND_PROMPT_${input.promptToken}.`,
...
describe("daemon E2E (real opencode) - rewind", () => {
  ...
  test("records exactly one plain-text user message for a real OpenCode turn", async () => {
    ...
    const prompt = "THOTH_OPENCODE_PLAIN_TEXT_DUP_CHECK. Reply exactly: OPENCODE_PLAIN_TEXT_DONE";
  ...
  test("rewinds a real OpenCode read-only turn without changing files", async () => {
    ...
    try {
      await harness.client.sendMessage(
        session.agentId,
        [
          "THOTH_OPENCODE_REWIND_PROMPT_READ_ONLY.",
  ...
  test("rewinds every file from a single real OpenCode multi-tool edit turn", async () => {
    ...
    try {
      await harness.client.sendMessage(
        session.agentId,
        [
          "THOTH_OPENCODE_REWIND_PROMPT_MULTI_EDIT.",
