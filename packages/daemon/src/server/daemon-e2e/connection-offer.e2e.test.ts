...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    process.env.THOTH_PRIMARY_LAN_IP = "192.168.1.12";
    ...
    try {
      ...
      expect(offerUrl.startsWith("https://app.thoth.seeles.ai/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.test.thoth.seeles.ai:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "thoth-offer-home-"));

    const { logger: logger1 } = createCapturingLogger();
    const daemon1 = await createTestThothDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      thothHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      ...
      const daemon2 = await createTestThothDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        thothHomeRoot: tempHomeRoot,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "thoth-offer-e2e-"));
