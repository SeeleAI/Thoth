...
function initGitRepo(repoDir: string): void {
  ...
  execSync("git config user.email 'thoth-test@example.com'", {
    cwd: repoDir,
    stdio: "pipe",
  });
  execSync("git config user.name 'Thoth Test'", {
    cwd: repoDir,
    stdio: "pipe",
  });
    ...
    async () => {
      ...
      try {
        ...
        const worktree = await createLegacyWorktreeForTest({
          ...
          thothHome: ctx.daemon.thothHome,
        });
        ...
        const worktreeList = await ctx.client.getThothWorktreeList({
          cwd: repoDir,
        });
        ...
        const archiveResult = await ctx.client.archiveThothWorktree({
        ...
        const worktreeListAfter = await ctx.client.getThothWorktreeList({
          cwd: repoDir,
        });
        expect(
