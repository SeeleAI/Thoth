...
class FakeHarnessThread implements HarnessThread {
  ...
  private async runSlashCommand(
    ...
    if (this.providerName === "codex" && fullName.startsWith("prompts:")) {
      ...
      return {
        text: `THOTH_OK ${args ?? ""}`.trim(),
        timeline: [{ type: "assistant_message", text: `THOTH_OK ${promptId}` }],
        usage: { inputTokens: 1, outputTokens: 1 },
      };
    }
    ...
    return {
      text: "THOTH_SKILL_OK",
      timeline: [{ type: "assistant_message", text: "THOTH_SKILL_OK" }],
      usage: { inputTokens: 1, outputTokens: 1 },
    };
  }
