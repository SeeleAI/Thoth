...
export interface ThothDaemonConfig {
  listen: string;
  thothHome: string;
...
function createBootstrapManagedProcessRegistry(
  config: Pick<ThothDaemonConfig, "thothHome" | "managedProcesses">,
  logger: Logger,
...
function mountWebUi(app: express.Application, config: ThothDaemonConfig, logger: Logger): void {
  app.use(
...
function resolveExpressTrustProxySetting(config: ThothDaemonConfig): true | string[] {
  return config.trustedProxies ?? ["loopback"];
}

export async function createThothDaemon(
  config: ThothDaemonConfig,
  rootLogger: Logger,
): Promise<ThothDaemon> {
  ...
  const allowedOrigins = new Set([
    ...
    // Packaged desktop renderers use the custom thoth:// protocol scheme.
    "thoth://app",
  ...
  const createThothWorktreeForTools = async (
    input: Parameters<typeof createThothWorktreeWorkflow>[1],
    serviceOptions?: Parameters<typeof createThothWorktreeWorkflow>[2],
  ) => {
    return createThothWorktreeWorkflow(
      {
        thothHome: config.thothHome,
        worktreesRoot: config.worktreesRoot,
        createThothWorktree: async (workflowInput, workflowOptions) => {
          return createRegisteredThothWorktree(workflowInput, {
            github,
  ...
  const createAgentToolHostDependencies = (
    ...
    createThothWorktree: createThothWorktreeForTools,
    thothHome: config.thothHome,
    worktreesRoot: config.worktreesRoot,
