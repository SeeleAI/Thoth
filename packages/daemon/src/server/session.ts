...
import { getErrorMessage, getErrorMessageOr } from "@thoth/protocol/error-utils";
...
import {
  ...
  createThothWorktreeWorkflow as createWorktreeWorkflow,
  type CreateThothWorktreeSetupContinuationInput,
  type CreateThothWorktreeWorkflowResult,
  handleCreateThothWorktreeRequest as handleCreateWorktreeRequest,
  handleThothWorktreeArchiveRequest as handleWorktreeArchiveRequest,
  handleThothWorktreeListRequest as handleWorktreeListRequest,
...
export class Session {
  ...
  constructor(options: SessionOptions) {
    ...
    this.createAgentLifecycleDispatch = new CreateAgentLifecycleDispatch({
      ...
      github: this.github,
      workspaceGitService: this.workspaceGitService,
      createThothWorktreeWorkflow: (input, workflowOptions) =>
        this.createThothWorktreeWorkflow(input, workflowOptions),
  ...
  private async buildAgentSessionConfig(
    ...
    setupContinuation?: CreateThothWorktreeWorkflowResult["setupContinuation"];
    ...
    return buildWorktreeAgentSessionConfig(
      {
        ...
        sessionLogger: this.sessionLogger,
        workspaceGitService: this.workspaceGitService,
        createThothWorktree: (input, serviceOptions) =>
          this.createThothWorktreeWorkflow(input, {
            ...serviceOptions,
            setupContinuation: {
              kind: "agent",
              terminalManager: this.terminalManager,
  ...
  private async handleThothWorktreeListRequest(
    msg: Extract<SessionInboundMessage, { type: "thoth_worktree_list_request" }>,
  ): Promise<void> {
  ...
  private async handleThothWorktreeArchiveRequest(
    msg: Extract<SessionInboundMessage, { type: "thoth_worktree_archive_request" }>,
  ): Promise<void> {
  ...
  private async describeCreatedWorktreeWorkspace(
    result: CreateThothWorktreeResult,
  ): Promise<WorkspaceDescriptorPayload> {
  ...
  private async recreateOwningWorktreeForRestore(
    ...
    // resolves repoRoot=null, so deleteThothWorktree's `git worktree remove`/
  ...
  private async createThothWorktree(
    input: CreateThothWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    },
  ): Promise<CreateThothWorktreeResult> {
    const result = await createThothWorktree(input, {
      github: this.github,
  ...
  private async handleWorkspaceCreateWorktree(
    ...
    const result = await this.createThothWorktreeWorkflow(
      {
        cwd: sourceCwd,
        projectId: source.projectId,
  ...
  private async handleCreateThothWorktreeRequest(
    request: Extract<SessionInboundMessage, { type: "create_thoth_worktree_request" }>,
  ): Promise<void> {
    return handleCreateWorktreeRequest(
      {
        ...
        emit: (message) => this.emit(message),
        sessionLogger: this.sessionLogger,
        createThothWorktreeWorkflow: (input) => this.createThothWorktreeWorkflow(input),
      },
      request,
    );
  }

  private async createThothWorktreeWorkflow(
    input: CreateThothWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
      setupContinuation?: CreateThothWorktreeSetupContinuationInput;
    },
    ...
    return createWorktreeWorkflow(
      {
        thothHome: this.thothHome,
        worktreesRoot: this.worktreesRoot,
        createThothWorktree: (workflowInput, serviceOptions) =>
          this.createThothWorktree(workflowInput, serviceOptions),
