...
export class DaemonConfigStore {
  ...
  constructor(thothHome: string, initial: MutableDaemonConfig, logger?: LoggerLike) {
  ...
  private persistConfig(config: MutableDaemonConfig): void {
    const persisted = loadPersistedConfig(this.thothHome, this.logger);
    const nextPersisted = mergeMutableConfigIntoPersistedConfig({
      persisted,
      mutable: config,
    });
    savePersistedConfig(this.thothHome, nextPersisted, this.logger);
  }
}
