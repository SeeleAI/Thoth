...
describe("wire compatibility", () => {
  ...
  test("legacy worktree request shape normalizes to the same internal input as the new shape", async () => {
    ...
    const dependencies = {
      ...
      createThothWorktreeWorkflow: workflow.create.bind(workflow),
    };
    ...
    if (legacyRequest.type !== "create_thoth_worktree_request") {
      throw new Error("Expected legacy worktree request");
    }
    if (newRequest.type !== "create_thoth_worktree_request") {
      throw new Error("Expected new worktree request");
    }

    await handleCreateThothWorktreeRequest(dependencies, legacyRequest);
    await handleCreateThothWorktreeRequest(dependencies, newRequest);
