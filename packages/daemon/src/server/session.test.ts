...
interface SessionHandlerInternals {
  ...
  createThothWorktree(params: unknown): Promise<unknown>;
...
const thothWorktreeServiceMocks = vi.hoisted(() => ({
  createThothWorktree: vi.fn(),
}));
...
vi.mock("./thoth-worktree-service.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    createThothWorktree: thothWorktreeServiceMocks.createThothWorktree,
  };
});
...
describe("workspace file access (behavior preservation)", () => {
  ...
  test("file upload round-trips bytes through binary frames", async () => {
    const thothHome = makeDir("file-access-upload-");
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({ messages, thothHome });
...
describe("project config RPC authorization", () => {
  ...
  test("read_project_config_request accepts the same root with a trailing slash", async () => {
    ...
    writeFileSync(join(repoRoot, "thoth.json"), JSON.stringify({ worktree: { setup: "npm ci" } }));
    const messages: unknown[] = [];
    const session = createSessionForTest({
      messages,
  ...
  test("write_project_config_request emits stale and write-failed inline domain failures", async () => {
    ...
    writeFileSync(join(staleRoot, "thoth.json"), JSON.stringify({ worktree: { setup: "old" } }));
...
`;
  ...
  function writeConfig(repoRoot: string, config: unknown): void {
    writeFileSync(join(repoRoot, "thoth.json"), `${JSON.stringify(config)}\n`);
  }

  async function generateCommitPromptWithConfig(config: unknown): Promise<string> {
    const repoRoot = makeRoot();
    if (typeof config === "string") {
      writeFileSync(join(repoRoot, "thoth.json"), config);
  ...
  test.each([
    ["thoth.json missing", undefined],
    ["thoth.json exists but invalid JSON", "{ nope"],
    ["thoth.json valid but missing metadataGeneration", {}],
...
describe("session branch validation", () => {
  ...
  test("does not validate tags as branches", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "thoth-session-branch-validation-"));
...
describe("session stash list handling", () => {
  test("lists stashes through the workspace git service", async () => {
    ...
    const entries = [
      {
        index: 0,
        message: "thoth-auto-stash: feature",
        branch: "feature",
        isThoth: true,
      },
    ];
...
describe("session thoth worktree creation handling", () => {
  test("forces workspace git refreshes for the source repo and created worktree", async () => {
    const workspaceGitService = { getSnapshot: vi.fn().mockResolvedValue({}) };
    const session = createSessionForTest({ workspaceGitService });
    thothWorktreeServiceMocks.createThothWorktree.mockResolvedValue({
      repoRoot: "/tmp/repo",
      worktree: {
        branchName: "feature/new-worktree",
        worktreePath: "/tmp/thoth/worktrees/new-worktree",
      },
      workspace: {
        workspaceId: "workspace-new-worktree",
        projectId: "project-repo",
        cwd: "/tmp/thoth/worktrees/new-worktree",
        kind: "worktree",
    ...
    await asSessionInternals(session).createThothWorktree({
      cwd: "/tmp/repo",
    ...
    expect(workspaceGitService.getSnapshot).toHaveBeenCalledWith(
      "/tmp/thoth/worktrees/new-worktree",
      {
        force: true,
        reason: "create-worktree",
      },
  ...
  test("passes service-owned git metadata into workspace script spawning", async () => {
    ...
    const workspaceGitService = {
      ...
      getWorkspaceGitMetadata: vi.fn().mockResolvedValue({
        projectKind: "git",
        projectDisplayName: "thoth/thoth",
...
describe("session pull request timeline handling", () => {
        ...
        items: [
          {
            ...
            number: 42,
            title: "Ship search",
            url: "https://github.com/thoth/thoth/pull/42",
            state: "OPEN",
            body: null,
            labels: [],
            baseRefName: "main",
        ...
        items: [
          {
            ...
            url: "https://github.com/thoth/thoth/pull/42#pullrequestreview-1",
  ...
  test.each([
    { prNumber: 0, repoOwner: "thoth", repoName: "thoth" },
    { prNumber: -1, repoOwner: "thoth", repoName: "thoth" },
    { prNumber: 42, repoOwner: "get thoth", repoName: "thoth" },
    { prNumber: 42, repoOwner: "thoth/cli", repoName: "thoth" },
    { prNumber: 42, repoOwner: "get$thoth", repoName: "thoth" },
    { prNumber: 42, repoOwner: "thoth", repoName: "pa seo" },
    { prNumber: 42, repoOwner: "thoth", repoName: "thoth/app" },
    { prNumber: 42, repoOwner: "thoth", repoName: "thoth!" },
  ...
  test("emits GitHub check details responses", async () => {
    ...
    const checkDetails: GitHubCheckDetails = {
      ...
      status: "completed",
      conclusion: "failure",
      url: "https://github.com/thoth/thoth/actions/runs/456/job/789",
      detailsUrl: "https://github.com/thoth/thoth/actions/runs/456/job/789",
