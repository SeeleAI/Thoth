...
type ChatMessageFixture = Awaited<ReturnType<WorkspaceChatService["dispatchMessage"]>>;

interface MakeOptions {
  chat?: { [K in keyof WorkspaceChatService]?: unknown };
...
function makeSubsystem(options: MakeOptions = {}) {
  ...
  const host: ChatScheduleSessionHost = {
    ...
    listLiveAgents: () => [],
    resolveAgentIdentifier: async (_workspaceId, identifier) => ({ ok: true, agentId: identifier }),
  ...
  const subsystem = new ChatScheduleSession({
    host,
    chatService: createStub<WorkspaceChatService>(options.chat ?? {}),
...
describe("ChatScheduleSession", () => {
  ...
  it("schedule/create remaps a self target to an agent target before creating", async () => {
    let received: Parameters<ScheduleService["create"]>[1] | undefined;
    const stored = {
      id: "s2",
      name: null,
      prompt: "p",
    ...
    const { subsystem, emitted } = makeSubsystem({
      schedule: {
        create: async (_workspaceId: string, input: Parameters<ScheduleService["create"]>[1]) => {
          received = input;
          return stored;
        },
      },
