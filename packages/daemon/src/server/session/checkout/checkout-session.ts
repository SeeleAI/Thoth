...
export class CheckoutSession {
  private static readonly THOTH_STASH_PREFIX = "thoth-auto-stash:";
  ...
  async handleStashSaveRequest(
    ...
    try {
      ...
      const message = branchLabel
        ? `${CheckoutSession.THOTH_STASH_PREFIX} ${branchLabel}`
        : `${CheckoutSession.THOTH_STASH_PREFIX} unnamed`;
  ...
  async handleStashListRequest(
    ...
    const { cwd, requestId } = msg;
    const thothOnly = msg.thothOnly !== false;
    try {
      const entries = await this.workspaceGitService.listStashes(cwd, { thothOnly });
  ...
  async handleCheckoutMergeRequest(
    ...
    try {
      ...
      const mutatedCwd = await mergeToBase(
        ...
        { thothHome: this.thothHome, worktreesRoot: this.worktreesRoot },
      );
      await Promise.all([
