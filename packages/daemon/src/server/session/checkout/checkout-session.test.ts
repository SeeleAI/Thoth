...
describe("CheckoutSession", () => {
  ...
  describe("stash list", () => {
    it("returns stash entries scoped to thoth stashes by default", async () => {
      const listStashesCalls: Array<{ cwd: string; thothOnly: boolean | undefined }> = [];
      const { checkout, emitted } = makeCheckoutSession({
        git: {
          listStashes: async (cwd, opts) => {
            listStashesCalls.push({ cwd, thothOnly: opts?.thothOnly });
            return [];
          },
        },
      });
      ...
      expect(listStashesCalls).toEqual([{ cwd: "/repo", thothOnly: true }]);
      expect(emitted).toEqual([
        {
