...
function makeSubsystem(
  ...
  const thothHome = makeDir("workspace-files-home-");
...
describe("WorkspaceFilesSession", () => {
  ...
  test("streams a file larger than the socket bound as paced independent chunks", async () => {
    ...
    expect(frames.at(0)?.metadata).toEqual(
      expect.objectContaining({ size: fileBytes.byteLength, revision: expect.any(String) }),
    );
  ...
  test("round-trips an upload through transfer frames", async () => {
    ...
    expect(readFileSync(join(thothHome, "uploads", "upload_req-upload", "notes.txt"), "utf8")).toBe(
      "hello world",
    );
  });
});
