...
import type { ProjectCheckoutLitePayload } from "@thoth/protocol/messages";
...
import { listThothWorktrees, type ThothWorktreeInfo } from "../utils/worktree.js";
...
const linuxWatchReaddirConcurrency =
  parseInt(process.env.THOTH_LINUX_WATCH_READDIR_CONCURRENCY ?? "16", 10) || 16;
...
export type WorkspaceGitWorktreeInfo = ThothWorktreeInfo;
...
interface WorkspaceGitServiceDependencies {
  ...
  listThothWorktrees: typeof listThothWorktrees;
...
export class WorkspaceGitServiceImpl implements WorkspaceGitService {
  ...
  async getCheckout(cwd: string): Promise<ProjectCheckoutLitePayload> {
    ...
    try {
      ...
      return checkoutLiteFromGitSnapshot(normalizedCwd, {
        ...
        isThothOwnedWorktree: status.isThothOwnedWorktree,
  ...
  listStashes(
    ...
    const normalizedCwd = resolve(cwd);
    const thothOnly = options?.thothOnly !== false;
    const key = JSON.stringify(["stashes", normalizedCwd, thothOnly]);
    return this.readAuxiliaryCache(this.stashListCache, key, readOptions, async () => {
      ...
      return parseWorkspaceGitStashList(stdout, { thothOnly });
    });
  }
  ...
  async resolveRepoRoot(cwd: string, options?: WorkspaceGitReadOptions): Promise<string> {
    ...
    return snapshot.git.isThothOwnedWorktree
  ...
  private async refreshGitSnapshot(
    ...
    target.latestGit = {
      ...
      isThothOwnedWorktree: checkoutStatus.isThothOwnedWorktree,
...
function parseWorkspaceGitStashList(
  ...
  for (const line of lines) {
    ...
    const prefix = "thoth-auto-stash:";
    const prefixIdx = subject.indexOf(prefix);
    const isThoth = prefixIdx >= 0;
    const branch = isThoth ? subject.slice(prefixIdx + prefix.length).trim() || null : null;

    if (options.thothOnly && !isThoth) {
      continue;
    }

    entries.push({ index, message: subject, branch, isThoth });
  }
