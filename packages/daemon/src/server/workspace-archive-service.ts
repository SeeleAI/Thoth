...
import {
  ...
  resolveThothWorktreeRootForCwd,
...
// Thoth-owned AND no active workspace still references it.
export async function archiveByScope(
  ...
  const { targetDir, targetWorkspaceIds } = await resolveArchiveTargets(
    dependencies,
    request.scope,
    request.thothWorktreesBaseRoot,
  );
...
async function resolveArchiveTargets(
  ...
  const resolvedWorktree = await resolveThothWorktreeRootForCwd(targetPath, {
    thothHome: dependencies.thothHome,
    worktreesRoot: thothWorktreesBaseRoot ?? dependencies.thothWorktreesBaseRoot,
  });
  if (resolvedWorktree) {
...
async function maybeRemoveDirectory(
  ...
  const ownership = await isThothOwnedWorktreeCwd(targetDir, {
    thothHome: dependencies.thothHome,
    worktreesRoot: request.thothWorktreesBaseRoot ?? dependencies.thothWorktreesBaseRoot,
  });
  ...
  try {
    await deleteThothWorktree({
      ...
      thothHome: dependencies.thothHome,
      worktreesBaseRoot: request.thothWorktreesBaseRoot ?? dependencies.thothWorktreesBaseRoot,
    });
