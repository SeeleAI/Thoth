...
export async function spawnWorkspaceScript(
  ...
  const configResult = readThothConfig(repoRoot);
  if (!configResult.ok) {
    throw thothConfigParseError(configResult);
  }
  const scriptConfigs = getScriptConfigs(configResult.config);
  const config = scriptConfigs.get(scriptName);
  if (!config) {
    throw new Error(`Script '${scriptName}' is not configured in thoth.json`);
  }
