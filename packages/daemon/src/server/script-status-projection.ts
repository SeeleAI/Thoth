...
import { getScriptConfigs, isServiceScript, readThothConfig } from "../utils/worktree.js";
...
export function readThothConfigForProjection(
  workspaceDirectory: string,
  logger: Logger,
): ThothConfig | null {
  const result = readThothConfig(workspaceDirectory);
  if (result.ok) {
    return result.config;
  }
  logger.warn(
    { configPath: result.configPath, workspaceDirectory, err: result.error },
    "Failed to parse thoth.json; treating workspace as having no scripts",
  );
  return null;
}
...
export function buildWorkspaceScriptPayloads(
  ...
  const scriptConfigs = getScriptConfigs(options.thothConfig);
