...
function seedSameCwdWorkspaces(): { thothHomeRoot: string; cwd: string } {
  const thothHomeRoot = mkdtempSync(path.join(tmpdir(), "thoth-same-cwd-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "thoth-same-cwd-dir-"));
  const projectsDir = path.join(thothHomeRoot, ".thoth", "projects");
  ...
  return { thothHomeRoot, cwd };
}

async function seedWorkspaceWithLegacyAgent(): Promise<{ thothHomeRoot: string; cwd: string }> {
  const thothHomeRoot = mkdtempSync(path.join(tmpdir(), "thoth-legacy-agent-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "thoth-legacy-agent-dir-"));
  const thothHome = path.join(thothHomeRoot, ".thoth");
  const projectsDir = path.join(thothHome, "projects");
  mkdirSync(projectsDir, { recursive: true });
  ...
  const agentStorage = new AgentStorage(path.join(thothHome, "agents"), createTestLogger());
...
test("daemon bootstrap migrates cwd-only legacy agents before same-cwd workspaces are added", async () => {
  const { thothHomeRoot, cwd } = await seedWorkspaceWithLegacyAgent();
  const daemon = await createTestThothDaemon({ thothHomeRoot });
...
test("local workspace auto-title does not broadcast provider snapshot warm-up to clients", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "thoth-title-snapshot-storm-"));
...
test("two workspaces sharing one cwd compute agent status per workspaceId", async () => {
  const { thothHomeRoot, cwd } = seedSameCwdWorkspaces();
  const daemon = await createTestThothDaemon({ thothHomeRoot });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
    appVersion: "0.1.82",
