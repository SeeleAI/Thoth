...
function createTempHome(): string {
  return mkdtempSync(path.join(tmpdir(), "thoth-config-"));
}
...
describe("PersistedConfigSchema worktrees config", () => {
  test("accepts optional worktree root", () => {
    const parsed = PersistedConfigSchema.parse({
      worktrees: {
        root: "/mnt/fast/thoth-worktrees",
      },
    });

    expect(parsed.worktrees?.root).toBe("/mnt/fast/thoth-worktrees");
  });
});
  ...
  test("accepts the documented config schema marker", () => {
    ...
    try {
      writeFileSync(
        ...
        `${JSON.stringify(
          {
            $schema: "https://thoth.sh/schemas/thoth.config.v1.json",
            version: 1,
            daemon: {
