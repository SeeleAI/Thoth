...
const CWD = "/tmp/thoth/worktrees/repo/branch";
const THOTH_HOME = "/tmp/thoth";
const WORKTREES_ROOT = "/tmp/thoth/worktrees/repo";
...
function createHarness(overrides?: {
  ...
  isThothOwnedWorktreeCwd?: ArchiveIfSafeDependencies["isThothOwnedWorktreeCwd"];
  ...
  const isThothOwnedWorktreeCwd = vi.fn(
    overrides?.isThothOwnedWorktreeCwd ??
      (async () => ({
        allowed: true,
        repoRoot: "/tmp/repo",
  ...
  ) as unknown as ArchiveIfSafeDependencies["isThothOwnedWorktreeCwd"];
...
function createGitRepo(): { tempDir: string; repoDir: string } {
  ...
  execFileSync("git", ["config", "user.name", "Thoth Test"], {
    cwd: repoDir,
    stdio: "pipe",
  });
...
async function createThothOwnedWorktree(
  repoDir: string,
  thothHome: string,
  worktreeSlug: string,
...
describe("archiveIfSafe", () => {
  ...
  test("does nothing when there is no snapshot", async () => {
    ...
    expect(harness.deps.isThothOwnedWorktreeCwd).not.toHaveBeenCalled();
    expect(harness.deps.archiveByScope).not.toHaveBeenCalled();
  });
  ...
  test("does nothing when the cwd is not a Thoth-owned worktree", async () => {
    const harness = createHarness({
      isThothOwnedWorktreeCwd: async () => ({ allowed: false, worktreePath: CWD }),
    });

    await runArchiveIfSafe(harness);

    expect(harness.deps.isThothOwnedWorktreeCwd).toHaveBeenCalledWith(CWD, {
      thothHome: THOTH_HOME,
    });
    expect(harness.deps.archiveByScope).not.toHaveBeenCalled();
  });
  ...
  test("archives a clean Thoth-owned worktree after merge", async () => {
    const harness = createHarness();
    ...
    expect(harness.deps.archiveByScope).toHaveBeenCalledWith(
      ...
      }),
      {
        ...
        repoRoot: "/tmp/repo",
        thothWorktreesBaseRoot: undefined,
  ...
  test("real outcome: keeps sibling workspace and directory on last reference", async () => {
    ...
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "merged-with-sibling");
  ...
  test("real outcome: removes directory when no sibling workspace remains", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const thothHome = path.join(tempDir, ".thoth");
    const worktree = await createThothOwnedWorktree(repoDir, thothHome, "merged-last-ref");
