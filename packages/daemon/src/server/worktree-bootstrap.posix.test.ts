  ...
  describe("runAsyncWorktreeBootstrap", () => {
    ...
    it("streams running setup updates live and persists only a final setup timeline row", async () => {
      ...
      const persistedSetupItems = persisted.filter(
        (item) => item.type === "tool_call" && item.name === "thoth_worktree_setup",
      );
    ...
    it("keeps only the final carriage-return-updated content in command logs", async () => {
      ...
      const persistedSetupItem = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" && item.name === "thoth_worktree_setup",
      );
    ...
    it("shares the same worktree runtime port across setup and bootstrap terminals", async () => {
      writeFileSync(
        ...
        JSON.stringify({
          worktree: {
            setup: ['echo "$THOTH_WORKTREE_PORT" > setup-port.txt'],
            terminals: [
              {
                name: "Port Terminal",
                command: "true",
      ...
      expect(registeredEnvs[0]?.env.THOTH_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalEnvs.length).toBeGreaterThan(0);
      expect(createTerminalEnvs[0]?.THOTH_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalWorkspaceIds).toEqual(["ws-shared-runtime-port"]);

      const terminalToolCall = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" &&
          item.name === "thoth_worktree_terminals" &&
    ...
    it("injects real peer service env into terminal-backed services", async () => {
      ...
      expect(apiEnv.THOTH_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:6767",
      );
      expect(apiEnv.THOTH_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:6767",
      );
      expect(apiEnv.THOTH_SERVICE_API_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.THOTH_SERVICE_WEB_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.THOTH_URL).toBe(apiEnv.THOTH_SERVICE_API_URL);
      expect(apiEnv.THOTH_PORT).toBe(apiEnv.THOTH_SERVICE_API_PORT);
      expect(apiEnv).not.toHaveProperty("PORT");

      expect(webEnv.THOTH_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:6767",
      );
      expect(webEnv.THOTH_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:6767",
      );
      expect(webEnv.THOTH_SERVICE_API_PORT).toBe(apiEnv.THOTH_SERVICE_API_PORT);
      expect(webEnv.THOTH_SERVICE_WEB_PORT).toBe(apiEnv.THOTH_SERVICE_WEB_PORT);
      expect(webEnv.THOTH_URL).toBe(webEnv.THOTH_SERVICE_WEB_URL);
      expect(webEnv.THOTH_PORT).toBe(webEnv.THOTH_SERVICE_WEB_PORT);
      expect(webEnv).not.toHaveProperty("PORT");

      const apiPort = Number(apiEnv.THOTH_SERVICE_API_PORT);
