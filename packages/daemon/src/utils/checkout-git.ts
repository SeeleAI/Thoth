...
import { isThothOwnedWorktreeCwd, resolveThothWorktreesBaseRoot } from "./worktree.js";
import { readThothWorktreeMetadata } from "./worktree-metadata.js";
...
export interface CheckoutStatusGitNonThoth {
  isGit: true;
  repoRoot: string;
...
export interface CheckoutStatusGitThoth {
  isGit: true;
  repoRoot: string;
  mainRepoRoot: string;
  currentBranch: string | null;
...
export type CheckoutStatusGit = CheckoutStatusGitNonThoth | CheckoutStatusGitThoth;
...
export type CheckoutSnapshotFacts =
  ...
  | {
      ...
      thothWorktree: ThothWorktreeForCwd;
...
async function getMainRepoRootFromCommonDir(
  ...
  const nonBareNonThoth = worktrees.filter(
    (wt) =>
      !wt.isBare &&
      !isThothWorktreePath(wt.path, {
        thothHome: context?.thothHome,
        worktreesRoot: context?.worktreesRoot,
      }),
  );
  const childrenOfBareRepo = nonBareNonThoth.filter((wt) => isDescendantPath(wt.path, normalized));
  const mainChild = childrenOfBareRepo.find((wt) => basename(wt.path) === "main");
  return mainChild?.path ?? childrenOfBareRepo[0]?.path ?? nonBareNonThoth[0]?.path ?? normalized;
}
...
/** Check whether a path is under Thoth's worktree root. */
export function isThothWorktreePath(
  p: string,
  options?: { thothHome?: string; worktreesRoot?: string },
): boolean {
  if (options?.worktreesRoot || options?.thothHome) {
    return isDescendantPath(p, resolveThothWorktreesBaseRoot(options));
  }
  return /[/\\]\.thoth[/\\]worktrees[/\\]/.test(p);
}
...
type ThothWorktreeForCwd =
  | { isThothOwnedWorktree: false }
  | { isThothOwnedWorktree: true; worktreeRoot: string };

async function getThothWorktreeForCwd(
  cwd: string,
  context?: CheckoutContext,
  knownWorktreeRoot?: string | null,
): Promise<ThothWorktreeForCwd> {
  // Fast-path reject: non-worktree paths do not need expensive ownership checks.
  if (!/[\\/]worktrees[\\/]/.test(cwd)) {
    return { isThothOwnedWorktree: false };
  }

  const ownership = await isThothOwnedWorktreeCwd(cwd, {
    thothHome: context?.thothHome,
    worktreesRoot: context?.worktreesRoot,
  });
...
function readThothWorktreeBaseRef(worktreeRoot: string): string | null {
  return readThothWorktreeMetadata(worktreeRoot)?.baseRefName ?? null;
}
...
async function getStoredBaseRefForCwd(
  ...
  const thothWorktree = await getThothWorktreeForCwd(cwd, context);
  if (!thothWorktree.isThothOwnedWorktree) {
    return null;
  }

  return readThothWorktreeBaseRef(thothWorktree.worktreeRoot);
}
...
async function inspectCheckoutContext(
  ...
  try {
    ...
    const [currentBranch, remoteUrl, absoluteGitDir, gitCommonDir, thothWorktree] =
      await Promise.all([
        getCurrentBranch(cwd),
        getOriginRemoteUrl(cwd),
        resolveAbsoluteGitDir(cwd),
        resolveGitCommonDir(cwd),
        getThothWorktreeForCwd(cwd, context, root),
      ]);
...
export async function getCheckoutSnapshotFacts(
  ...
  const storedBaseRef = inspected.thothWorktree.isThothOwnedWorktree
    ? readThothWorktreeBaseRef(inspected.thothWorktree.worktreeRoot)
    : null;
  ...
  return {
    ...
    thothWorktree: inspected.thothWorktree,
    storedBaseRef,
    resolvedBaseRef,
    mainRepoRoot,
    comparisonBaseRef,
...
export async function getCheckoutStatus(
  ...
  const thothWorktree = facts.thothWorktree;
  ...
  if (thothWorktree.isThothOwnedWorktree && baseRef) {
    return {
      isGit: true,
