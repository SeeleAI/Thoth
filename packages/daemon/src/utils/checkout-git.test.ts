...
async function readPullRequestLookupTargetFromFacts(
  ...
  const facts = await getCheckoutSnapshotFacts(repoDir, { thothHome });
  if (!facts.isGit) {
...
describe("checkout git utilities", () => {
  ...
  it("reuses checkout snapshot facts across status, shortstat, and PR status reads", async () => {
    ...
    const facts = await getCheckoutSnapshotFacts(repoDir, { thothHome });
    const status = await getCheckoutStatus(repoDir, { thothHome, facts });
    const shortstat = await getCheckoutShortstat(repoDir, { thothHome, facts }, { force: true });
    ...
    const statusWithFacts = await getCheckoutStatus(repoDir, { thothHome, facts });
...
`;
  ...
  it("returns checkout root metadata for normal repos", async () => {
    ...
    expect(status.isThothOwnedWorktree).toBe(false);
  ...
  it("reports a PR worktree as not ahead when its branch is pushed to the configured PR remote", async () => {
    ...
    commitFile(repoDir, "feature.txt", "feature\n", "feature commit");
    execFileSync("git", ["remote", "add", "thoth-pr-1285", prRemoteDir], { cwd: repoDir });
    execFileSync(
      "git",
      ["push", "thoth-pr-1285", "HEAD:refs/heads/open-button-targets-active-file"],
      { cwd: repoDir },
    );
    execFileSync(
      "git",
      ["config", "branch.aaronzhongg/open-button-targets-active-file.remote", "thoth-pr-1285"],
      {
        cwd: repoDir,
      },
    );
  ...
  it("reports a PR worktree as behind when its configured PR remote has newer commits", async () => {
    ...
    execFileSync("git", ["fetch", "thoth-pr-1285"], { cwd: repoDir });
  ...
  it("does not report full history as unpushed for fresh no-track Thoth worktrees", async () => {
    setupRemoteTrackingMain(repoDir, tempDir);
    ...
    const status = await getCheckoutStatus(worktree.worktreePath, { thothHome });
    expect(status).toMatchObject({
      isGit: true,
  ...
  it("handles status/diff/commit in a .thoth worktree", async () => {
    ...
    const status = await getCheckoutStatus(result.worktreePath, { thothHome });
    expect(status.isGit).toBe(true);
    expect(realpathSync.native(status.repoRoot)).toBe(realpathSync.native(result.worktreePath));
    expect(status.isDirty).toBe(true);
    expect(status.isThothOwnedWorktree).toBe(true);
    expect(realpathSync.native(status.mainRepoRoot ?? "")).toBe(realpathSync.native(repoDir));

    const diff = await getCheckoutDiff(result.worktreePath, { mode: "uncommitted" }, { thothHome });
    expect(diff.diff).toContain("-hello");
    expect(diff.diff).toContain("+worktree change");

    await commitAll(result.worktreePath, "worktree update");

    const cleanStatus = await getCheckoutStatus(result.worktreePath, { thothHome });
    expect(cleanStatus.isDirty).toBe(false);
  ...
  it("returns checkout root metadata for .thoth worktrees", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("detects plain git worktrees from git alone", async () => {
    ...
    const status = await getCheckoutStatus(worktreeDir, { thothHome });
    expect(status.isGit).toBe(true);
  ...
  it("merges the current branch into base from a worktree checkout", async () => {
    ...
    await mergeToBase(worktree.worktreePath, { baseRef: "main" }, { thothHome });
    ...
    const statusAfterMerge = await getCheckoutStatus(worktree.worktreePath, { thothHome });
  ...
  it("reports the base worktree cwd when merge-to-base mutates a separate checkout", async () => {
    ...
    const mutatedCwd = await mergeToBase(featureWorktree.worktreePath, {}, { thothHome });
  ...
  it("pushes the current branch to its configured upstream", async () => {
    ...
    execFileSync("git", ["remote", "add", "thoth-pr-526", prRemoteDir], { cwd: repoDir });
    execFileSync("git", ["checkout", "-b", "therainisme/main"], { cwd: repoDir });
    execFileSync("git", ["fetch", "thoth-pr-526", "main"], { cwd: repoDir });
    execFileSync("git", ["config", "branch.therainisme/main.remote", "thoth-pr-526"], {
      cwd: repoDir,
    });
    ...
    expect(upstream).toBe("thoth-pr-526/main");
  });
  ...
  it("pushes the current branch to its configured push remote", async () => {
    ...
    execFileSync("git", ["remote", "add", "thoth-pr-526", prRemoteDir], { cwd: repoDir });
    execFileSync("git", ["checkout", "-b", "therainisme/main"], { cwd: repoDir });
    execFileSync("git", ["config", "branch.therainisme/main.pushRemote", "thoth-pr-526"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.thoth-pr-526.push", "HEAD:refs/heads/main"], {
      cwd: repoDir,
    });
    writeFileSync(join(repoDir, "fork-pr.txt"), "fork pr edit\n");
    execFileSync("git", ["add", "fork-pr.txt"], { cwd: repoDir });
    ...
    const trackedPrRemoteHead = execFileSync(
      "git",
      ["rev-parse", "refs/remotes/thoth-pr-526/main"],
      {
        cwd: repoDir,
      },
    )
    ...
    expect(prRemoteMain).toBe(localHead);
    expect(getBranchUpstream(repoDir)).toBe("thoth-pr-526/main");
  ...
  it("refreshes the tracked ref after pushing through a configured push remote", async () => {
    ...
    execFileSync("git", ["checkout", "-b", "feature"], { cwd: repoDir });
    execFileSync("git", ["push", "-u", "origin", "feature"], { cwd: repoDir });
    execFileSync("git", ["remote", "add", "thoth-pr-1790", originDir], { cwd: repoDir });
    execFileSync("git", ["config", "branch.feature.pushRemote", "thoth-pr-1790"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.thoth-pr-1790.push", "HEAD:refs/heads/feature"], {
      cwd: repoDir,
    });
  ...
  it("disables GitHub features when gh is unavailable", async () => {
    execFileSync("git", ["remote", "add", "origin", "https://github.com/thoth/thoth.git"], {
      cwd: repoDir,
    });
          ...
          checks: [
            {
              name: "server-tests",
              status: "success",
              url: "https://github.com/thoth/thoth/actions/runs/123",
              workflow: "Server CI",
              duration: "2m 14s",
            },
          ],
  ...
  it("uses an origin tracked head when the local branch name differs", async () => {
    ...
    const lookupTarget = await readPullRequestLookupTargetFromFacts(repoDir, thothHome);
  ...
  it("does not attach an owner when the tracked remote is the same GitHub repository", async () => {
    ...
    execFileSync("git", ["remote", "add", "origin", "git@github.com:thoth/thoth.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["remote", "add", "upstream", "https://github.com/thoth/thoth.git"], {
      cwd: repoDir,
    });
  ...
  it("keeps the fork owner when same-repo comparison is indeterminate", async () => {
    ...
    execFileSync("git", ["remote", "add", "thoth-pr-345", "git@github.com:chethanuk/thoth.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "branch.chethanuk/main.remote", "thoth-pr-345"], {
      cwd: repoDir,
    });
  ...
  it("uses the configured push remote for fork PR lookup when upstream is absent", async () => {
    ...
    execFileSync("git", ["config", "branch.chethanuk/main.pushRemote", "thoth-pr-345"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.thoth-pr-345.push", "HEAD:refs/heads/main"], {
      cwd: repoDir,
    });
    ...
    const factsTarget = await readPullRequestLookupTargetFromFacts(repoDir, thothHome);
  ...
  it("uses the tracked fork branch for PR worktree status lookup", async () => {
    ...
    const github = createGitHubServiceRecordingPullRequestTargets({
      ...
      statusOverrides: {
        number: 345,
        url: "https://github.com/thoth/thoth/pull/345",
      },
    });
  ...
  it("returns closed-unmerged PR status without marking it as merged", async () => {
    ...
    const status = await getPullRequestStatus(
      ...
      createGitHubServiceForStatus(
        createPullRequestStatus({
          url: "https://github.com/thoth/thoth/pull/999",
  ...
  it("expires cached PR status after the TTL", async () => {
    ...
    try {
      ...
      github.getCurrentPullRequestStatus = async () => {
        callCount += 1;
        return createPullRequestStatus({
          url: `https://github.com/thoth/thoth/pull/${callCount}`,
        });
      };
      const first = await getPullRequestStatus(repoDir, github);
      await sleep(80);
  ...
  it("uses stored baseRefName for Thoth worktrees (no heuristics)", async () => {
    ...
    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { thothHome });
  ...
  it("names both refs when a requested base ref does not match the stored one", async () => {
    ...
    await expect(
      getCheckoutDiff(worktree.worktreePath, { mode: "base", baseRef: "other" }, { thothHome }),
    ).rejects.toThrow("Base ref mismatch: stored main, requested other");
  });

  it("excludes dirty working tree changes from Thoth worktree base diffs", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("merges to stored baseRefName when baseRef is not provided", async () => {
    ...
    // Create a Thoth worktree configured to use develop as base.
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "develop",
    ...
    await mergeToBase(worktree.worktreePath, {}, { thothHome });
  ...
  it("falls back to the repository default branch for base-dependent operations when metadata is missing", async () => {
    ...
    const metadataPath = getThothWorktreeMetadataPath(worktree.worktreePath);
    rmSync(metadataPath, { force: true });

    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { thothHome });
    expect(baseDiff.diff).toContain("feature.txt");

    const shortstat = await getCheckoutShortstat(worktree.worktreePath, { thothHome });
    expect(shortstat).toEqual({ additions: 1, deletions: 0 });
  });

  it("falls back to plain git checkout status when Thoth worktree metadata is missing", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  describe("parseWorktreeList", () => {
    it("parses porcelain worktree output", () => {
      const output = [
        ...
        "",
        "worktree /home/user/.thoth/worktrees/feature",
      ...
      expect(entries[1]).toEqual({
        path: "/home/user/.thoth/worktrees/feature",
  ...
  describe("isThothWorktreePath", () => {
    it("matches Unix .thoth/worktrees/ paths", () => {
      expect(isThothWorktreePath("/home/user/.thoth/worktrees/feature")).toBe(true);
    });

    it("matches Windows .thoth\\worktrees\\ paths", () => {
      expect(isThothWorktreePath("C:\\Users\\dev\\.thoth\\worktrees\\feature")).toBe(true);
    });

    it("matches worktrees under a custom THOTH_HOME", () => {
      const customThothHome = process.platform === "win32" ? "C:\\thoth" : "/var/lib/thoth";
      const worktreePath =
        process.platform === "win32"
          ? win32.join(customThothHome, "worktrees", "project", "feature")
          : `${customThothHome}/worktrees/project/feature`;

      expect(
        isThothWorktreePath(worktreePath, {
          thothHome: customThothHome,
        }),
      ).toBe(true);
    });

    it("rejects paths without .thoth/worktrees segment", () => {
      expect(isThothWorktreePath("/home/user/repo")).toBe(false);
      expect(isThothWorktreePath("C:\\Users\\dev\\repo")).toBe(false);
    });
  });
