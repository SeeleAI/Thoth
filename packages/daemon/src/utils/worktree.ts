...
export {
  ...
  ThothScriptEntryRawSchema,
  ThothWorktreeConfigRawSchema,
  ThothConfigSchema,
  type ThothConfig,
  type ThothConfigRaw,
} from "@thoth/protocol/thoth-config-schema";
import { ThothConfigSchema, type ThothConfig } from "@thoth/protocol/thoth-config-schema";
import {
  normalizeBaseRefName,
  readThothWorktreeMetadata,
  readThothWorktreeRuntimePort,
  writeThothWorktreeMetadata,
...
export interface WorktreeRuntimeEnv {
  [key: string]: string;
  THOTH_SOURCE_CHECKOUT_PATH: string;
  THOTH_ROOT_PATH: string;
  THOTH_WORKTREE_PATH: string;
  THOTH_BRANCH_NAME: string;
  THOTH_WORKTREE_PORT: string;
}
...
export interface ThothWorktreeInfo {
  path: string;
  createdAt: string;
  branchName?: string;
  head?: string;
...
export interface ThothWorktreeOwnership {
  allowed: boolean;
  repoRoot?: string;
  worktreeRoot?: string;
  worktreePath?: string;
...
export type ReadThothConfigResult =
  | { ok: true; config: ThothConfig | null }
  | { ok: false; configPath: string; error: unknown };

export function readThothConfig(repoRoot: string): ReadThothConfigResult {
  try {
    const json = readThothConfigJson(repoRoot);
    if (json === null) {
      return { ok: true, config: null };
    }
    return { ok: true, config: ThothConfigSchema.parse(json) };
  } catch (error) {
    return { ok: false, configPath: resolveThothConfigPath(repoRoot), error };
  }
}

export function thothConfigParseError(failure: { configPath: string; error: unknown }): Error {
  const detail = failure.error instanceof Error ? failure.error.message : String(failure.error);
  return new Error(`Failed to parse thoth.json at ${failure.configPath}: ${detail}`, {
    cause: failure.error,
  });
}

function readThothConfigOrThrow(repoRoot: string): ThothConfig | null {
  const result = readThothConfig(repoRoot);
  if (!result.ok) {
    throw thothConfigParseError(result);
  }
  return result.config;
}

export function getWorktreeSetupCommands(repoRoot: string): string[] {
  return readThothConfigOrThrow(repoRoot)?.worktree?.setup ?? [];
}

export function getWorktreeTeardownCommands(repoRoot: string): string[] {
  return readThothConfigOrThrow(repoRoot)?.worktree?.teardown ?? [];
}

export function getWorktreeTerminalSpecs(repoRoot: string): WorktreeTerminalConfig[] {
  const terminals = readThothConfigOrThrow(repoRoot)?.worktree?.terminals;
...
export function getScriptConfigs(config: ThothConfig | null): Map<string, ScriptConfig> {
...
export async function runWorktreeSetupCommands(options: {
  ...
  // Read thoth.json from the worktree (it will have the same content as the source repo)
...
export async function resolveWorktreeRuntimeEnv(options: {
  ...
  let worktreePort = readThothWorktreeRuntimePort(options.worktreePath);
  if (worktreePort === null) {
    worktreePort = await getAvailablePort();
    const metadata = readThothWorktreeMetadata(options.worktreePath);
    if (metadata) {
      writeThothWorktreeRuntimeMetadata(options.worktreePath, { worktreePort });
    }
  } else {
  ...
  return {
    ...
    // source checkout.
    THOTH_SOURCE_CHECKOUT_PATH: repoRootPath,
    // Backward-compatible alias.
    THOTH_ROOT_PATH: repoRootPath,
    THOTH_WORKTREE_PATH: options.worktreePath,
    THOTH_BRANCH_NAME: branchName,
    THOTH_WORKTREE_PORT: String(worktreePort),
  };
}
...
export async function runWorktreeTeardownCommands(options: {
  ...
  const branchName =
    options.branchName ?? (await resolveBranchNameForWorktreePath(options.worktreePath));
  const worktreePort = readThothWorktreeRuntimePort(options.worktreePath);
  ...
  const teardownEnv: NodeJS.ProcessEnv = createExternalProcessEnv(process.env, {
    ...
    THOTH_WORKTREE_PATH: options.worktreePath,
    THOTH_BRANCH_NAME: branchName,
    ...(worktreePort !== null ? { THOTH_WORKTREE_PORT: String(worktreePort) } : {}),
  });
...
export function resolveThothWorktreesBaseRoot(options?: WorktreeRootOptions): string {
  if (options?.worktreesRoot) {
    ...
    const home = options.thothHome ? resolve(options.thothHome) : resolveThothHome();
...
export async function getThothWorktreesRoot(
  cwd: string,
  thothHome?: string,
  worktreesRoot?: string,
): Promise<string> {
  const baseRoot = resolveThothWorktreesBaseRoot({ thothHome, worktreesRoot });
...
export async function computeWorktreePath(
  ...
  const projectWorktreesRoot = await getThothWorktreesRoot(cwd, thothHome, worktreesRoot);
...
export async function isThothOwnedWorktreeCwd(
  cwd: string,
  options?: WorktreeRootOptions,
): Promise<ThothWorktreeOwnership> {
  ...
  const worktreesBaseRoot = resolveThothWorktreesBaseRoot(options);
  const thothWorktreesPrefix = normalizePathForOwnership(worktreesBaseRoot) + sep;

  // Ownership is defined by the path living under <worktrees-root>/<hash>/<slug>[/...].
  // The <hash>/<slug> prefix is Thoth-private — nothing else writes there — so the
  // path shape alone is sufficient proof of ownership, even when git has already
  // forgotten about the worktree.
  if (!resolvedCwd.startsWith(thothWorktreesPrefix)) {
    return {
      allowed: false,
  ...
  const relative = resolvedCwd.slice(thothWorktreesPrefix.length);
...
type ParsedThothWorktreeInfo = Omit<ThothWorktreeInfo, "createdAt">;

function parseWorktreeList(output: string): ParsedThothWorktreeInfo[] {
  const entries: ParsedThothWorktreeInfo[] = [];
  let current: ParsedThothWorktreeInfo | null = null;
...
export async function listThothWorktrees({
  cwd,
  thothHome,
  worktreesRoot,
}: {
...
export async function resolveExistingWorktreeForSlug({
  ...
  const worktrees = await listThothWorktrees({
    cwd: repoRoot,
    thothHome,
    worktreesRoot,
  });
...
export async function resolveThothWorktreeRootForCwd(
  cwd: string,
  options?: WorktreeRootOptions,
  ...
  const knownWorktrees = await listThothWorktrees({
    cwd,
...
export async function deleteThothWorktree({
  cwd,
  worktreePath,
  worktreeSlug,
  worktreesRoot,
  ...
  if (worktreesRoot) {
    ...
    resolvedWorktreesRoot = await getThothWorktreesRoot(cwd, thothHome, worktreesBaseRoot);
  } else {
    throw new Error("cwd or worktreesRoot is required to delete a Thoth worktree");
  }
  ...
  const resolvedWorktree =
    (
      await resolveThothWorktreeRootForCwd(requestedPath, {
        thothHome,
  ...
  if (!resolvedWorktree.startsWith(resolvedRoot)) {
    throw new Error("Refusing to delete non-Thoth worktree");
  }
...
export const createWorktree = async ({
  ...
  let worktreePath = join(await getThothWorktreesRoot(cwd, thothHome, worktreesRoot), worktreeSlug);
  ...
  writeThothWorktreeMetadata(worktreePath, { baseRefName: sourcePlan.metadataBaseRefName });

  // If thoth.json exists in the main repo but wasn't checked into the worktree
  ...
  const mainConfigPath = join(cwd, "thoth.json");
  const worktreeConfigPath = join(worktreePath, "thoth.json");
  try {
  ...
  switch (source.kind) {
    ...
    case "checkout-github-pr": {
      ...
      if (source.pushRemoteUrl) {
        const remoteName = `thoth-pr-${source.githubPrNumber}`;
        ...
        if (originUrl) {
          remotePlan.pushRemote = {
            name: `thoth-pr-${source.githubPrNumber}`,
            url: originUrl,
            headRef: source.headRef,
            track: false,
          };
...
function normalizeRequiredBaseBranch(baseBranch: string): string {
  ...
  if (!normalizedBaseBranch) {
    throw new Error("Base branch is required when creating a Thoth worktree");
  }
  if (normalizedBaseBranch === "HEAD") {
    throw new Error("Base branch cannot be HEAD when creating a Thoth worktree");
  }
