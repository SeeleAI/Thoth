...
import { createTestThothDaemon } from "../src/server/test-utils/thoth-daemon.js";
...
async function verifyInjectedMcpForProvider(
  ...
  try {
    const prompt = [
      ...
      "If you have a tool called list_agents or create_agent from a thoth MCP server, call list_agents once.",
      "After checking, reply with exactly THOTH_MCP_FOUND.",
      "If you do not have those tools, reply with exactly THOTH_MCP_NOT_FOUND.",
    ...
    if (!assistantText.includes("THOTH_MCP_FOUND")) {
      throw new Error(
        `Expected assistant to confirm Thoth MCP availability. Assistant text:\n${assistantText}`,
      );
    }
...
async function main(): Promise<void> {
  ...
  const rootCwd = await mkdtemp(path.join(os.tmpdir(), "thoth-mcp-inject-real-"));
