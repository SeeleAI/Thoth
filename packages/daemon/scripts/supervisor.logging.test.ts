...
async function runSupervisorFixture(options: {
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "thoth-supervisor-log-"));
...
describe("supervisor durable logging", () => {
  test("resolves rotation defaults", () => {
    const thothHome = path.join(path.sep, "tmp", "thoth-home");
    const logFile = resolveSupervisorLogFile(thothHome, {}, {});

    expect(logFile).toEqual({
      path: path.join(thothHome, "daemon.log"),
  ...
  test("lets persisted rotation override env rotation defaults", () => {
    ...
    const logFile = resolveSupervisorLogFile(
      ...
      {
        THOTH_LOG_ROTATE_SIZE: "200m",
        THOTH_LOG_ROTATE_COUNT: "12",
      },
    );

    expect(logFile).toEqual({
      path: path.resolve(thothHome, "logs", "daemon.log"),
      rotate: { maxSize: "25m", maxFiles: 4 },
    });
  });
  ...
  test("uses env rotation when persisted rotation is absent", () => {
    ...
    const logFile = resolveSupervisorLogFile(
      ...
      {},
      {
        THOTH_LOG_ROTATE_SIZE: "50m",
        THOTH_LOG_ROTATE_COUNT: "8",
      },
    );
  ...
  test("logs the worker shutdown reason before signaling the worker", async () => {
    const result = await runSupervisorFixture({
      workerSource: `
        process.send?.({ type: "thoth:shutdown", reason: "client_shutdown_rpc" });
        setInterval(() => {}, 1000);
      `,
    });
