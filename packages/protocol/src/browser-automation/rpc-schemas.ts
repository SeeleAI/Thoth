...
export type BrowserAutomationToolName =
  `browser_${(typeof BROWSER_AUTOMATION_COMMAND_NAMES)[number]}`;
export const BROWSER_AUTOMATION_TOOL_NAMES = Object.freeze(
  BROWSER_AUTOMATION_COMMAND_NAMES.map(
    (command): BrowserAutomationToolName => `browser_${command}`,
  ),
);
