...
import { generateLocalPairingOffer, loadConfig, resolveThothHome } from "@thoth/daemon";
...
export async function runPairCommand(options: PairOptions): Promise<void> {
  if (options.home) {
    process.env.THOTH_HOME = options.home;
  }

  const thothHome = resolveThothHome();
  const state = resolveLocalDaemonState({ home: thothHome });
