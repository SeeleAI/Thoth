...
const QR = "\u001b[47m\u001b[30m      \n ████ \n      \u001b[0m";
const URL = "https://app.thoth.seeles.ai/#offer=pairing-offer";
...
describe("formatPairingInstructions", () => {
  ...
  it("keeps the pairing link available when QR rendering failed", () => {
    const output = formatPairingInstructions({ qr: null, url: URL, columns: 120 });

    expect(output).toContain("QR code is unavailable");
    expect(output.split("\n")).toContain(URL);
  });
});
