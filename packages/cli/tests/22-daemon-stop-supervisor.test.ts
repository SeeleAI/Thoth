...
/**
 * Regression: `thoth daemon stop` must stop supervised dev daemons
...
async function readPidLockState(thothHome: string): Promise<PidLockState> {
...
async function readCapturedSupervisorLogs(thothHome: string, recentLogs: string): Promise<string> {
  const durableLogs = await readFile(join(thothHome, "daemon.log"), "utf8").catch(() => "");
  return `${recentLogs}\n${durableLogs}`;
}
...
const thothHome = await mkdtemp(join(tmpdir(), "thoth-stop-supervisor-"));
...
try {
  console.log("Test 1: start supervisor-entrypoint in dev mode with isolated THOTH_HOME");
  ...
  const pidLockBeforeStop = await readPidLockState(thothHome);
  ...
  const pidAfterStop = await readPidLockState(thothHome);
  ...
  const statusAfterStop = await readDaemonStatus(thothHome);
  assert.strictEqual(
    statusAfterStop.localDaemon,
    "stopped",
    "daemon should remain stopped after stop command",
  );
  const capturedSupervisorLogs = await readCapturedSupervisorLogs(thothHome, recentSupervisorLogs);
  assert(
