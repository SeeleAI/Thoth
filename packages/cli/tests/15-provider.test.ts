...
function assertClaudeModels(data: ProviderModel[]): void {
  assert(
    data.length >= EXPECTED_CLAUDE_MODELS.length,
    "claude output should include the complete built-in catalog",
  );
...
try {
  // Test 1: provider --help shows subcommands
  {
    ...
    const result = await ctx.thoth(["provider", "--help"]);
  ...
  // Test 2: provider ls lists all providers
  {
    ...
    const result = await ctx.thoth(["provider", "ls"]);
  ...
  // Test 3: provider ls --json outputs valid JSON
  {
    ...
    const result = await ctx.thoth(["provider", "ls", "--json"]);
  ...
  // Test 4: provider ls includes disabled providers
  {
    ...
    const disabledCtx = await startTestDaemon({ thothHome, workDir, timeout: 120000 });
    try {
      const result = await runThothCli(disabledCtx, ["provider", "ls", "--json"]);
      assert.strictEqual(result.exitCode, 0, "provider ls should exit 0");
      ...
      const modelsResult = await runThothCli(disabledCtx, ["provider", "models", "claude"]);
  ...
  // Test 5: provider ls --quiet outputs provider names only
  {
    ...
    const result = await ctx.thoth(["provider", "ls", "--quiet"]);
    assert.strictEqual(result.exitCode, 0, "should exit 0");
  ...
  // Test 9: provider models unknown fails with error
  {
    ...
    const result = await ctx.thoth(["provider", "models", "unknown"]);
  ...
  // Test 11: provider models --quiet outputs model IDs only
  {
    ...
    if (claudeModelsProbe.available) {
      ...
      assert.strictEqual(result.exitCode, 0, "should exit 0");
      const lines = result.stdout.trim().split("\n").filter(Boolean);
      assert.strictEqual(lines.length, claudeModelIdsFromJson.length);
