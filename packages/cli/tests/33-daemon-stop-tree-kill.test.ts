...
const thothHome = await mkdtemp(join(tmpdir(), "thoth-stop-tree-kill-"));
const childPidPath = join(thothHome, "descendant.pid");
...
try {
  ...
  const stopResult =
    await $`THOTH_HOME=${thothHome} npx thoth daemon stop --home ${thothHome} --json --timeout 1 --force --kill-timeout 2`.nothrow();
