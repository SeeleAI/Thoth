...
/**
 * Test runner for Thoth CLI E2E tests
 *
 ...
 * its own daemon (ephemeral port + tmp THOTH_HOME), so parallelism is safe.
 */
...
const shardTotal = parsePositiveInt(process.env.THOTH_CLI_TEST_SHARD_TOTAL, 1);
const shardIndexRaw = parsePositiveInt(process.env.THOTH_CLI_TEST_SHARD, 1);
if (shardIndexRaw < 1 || shardIndexRaw > shardTotal) {
  throw new Error(
    `THOTH_CLI_TEST_SHARD=${shardIndexRaw} out of range for SHARD_TOTAL=${shardTotal}`,
  );
}
...
async function writeJsonSummary({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        suite: "cli-local",
        command: "npm run test:local --workspace=@thoth/cli",
        counts: {
          passed,
          failed,
          skipped: 0,
...
console.log("🧪 Thoth CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
async function runSingleTest(testFile: string): Promise<TestOutcome> {
  ...
  const npmCache = await mkdtemp(join(tmpdir(), "thoth-cli-test-npm-cache-"));
