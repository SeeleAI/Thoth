...
function getCliShimPath(appPath) {
  if (process.platform === "darwin") {
    return path.join(appPath, "Contents", "Resources", "bin", "thoth");
  }

  if (process.platform === "win32") {
    return path.join(appPath, "resources", "bin", "thoth.cmd");
  }

  return path.join(appPath, "resources", "bin", "thoth");
}
...
async function smokeColdCliDaemonStart({ appPath }) {
  const home = createTempDir("thoth-smoke-cli-daemon-home-");
  const pidPath = path.join(home, "thoth.pid");
...
async function smokeCliTerminal({ appPath, env }) {
  const cwd = createTempDir("thoth-smoke-terminal-cwd-");
  const marker = `thoth-packaged-terminal-smoke-${Date.now()}`;
