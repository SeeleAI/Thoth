...
describe("ThothBrowserWebviewRegistry", () => {
  it("keeps one authoritative webContents target per host and browser", () => {
    const registry = new ThothBrowserWebviewRegistry();
  ...
  it("keeps the first workspace binding and ignores cross-workspace active claims", () => {
    const registry = new ThothBrowserWebviewRegistry();
    registry.registerWebContents({
      webContentsId: 11,
      browserId: "browser-a",
    ...
    expect(registry.registerWorkspace({ browserId: "browser-a", workspaceId: "workspace-a" })).toBe(
      true,
    );
    expect(registry.registerWorkspace({ browserId: "browser-a", workspaceId: "workspace-b" })).toBe(
      false,
    );
    registry.setWorkspaceActiveBrowser({
      hostWebContentsId: 101,
    ...
    expect(registry.getWorkspaceId("browser-a")).toBe("workspace-a");
    expect(registry.getActiveBrowserIdForWorkspaceInHostWindow(101, "workspace-b")).toBeNull();
  });
});
