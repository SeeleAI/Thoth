...
export class ThothBrowserWebviewRegistry {
  ...
  public registerWorkspace(input: BrowserWorkspaceRegistration): boolean {
    const currentWorkspaceId = this.workspaceIdsByBrowserId.get(input.browserId);
    if (currentWorkspaceId && currentWorkspaceId !== input.workspaceId) {
      return false;
    }
    this.workspaceIdsByBrowserId.set(input.browserId, input.workspaceId);
    this.deleteCrossWorkspaceActiveBrowserReferences(input.browserId, input.workspaceId);
    return true;
  }
  ...
  public setWorkspaceActiveBrowser(input: {
    ...
    if (this.hasBrowser(input.browserId)) {
      if (!this.registerWorkspace({ browserId: input.browserId, workspaceId: input.workspaceId })) {
        return;
      }
    }
  ...
  private deleteCrossWorkspaceActiveBrowserReferences(
    browserId: string,
    workspaceId: string,
  ): void {
    for (const [hostWebContentsId, activeBrowserIdsByWorkspace] of this
      .activeBrowserIdsByHostWindow) {
      for (const [activeWorkspaceId, activeBrowserId] of activeBrowserIdsByWorkspace) {
        if (activeBrowserId === browserId && activeWorkspaceId !== workspaceId) {
          activeBrowserIdsByWorkspace.delete(activeWorkspaceId);
        }
      }
      if (activeBrowserIdsByWorkspace.size === 0) {
        this.activeBrowserIdsByHostWindow.delete(hostWebContentsId);
