...
describe("browser automation IPC adapter", () => {
  ...
  test("invalidates every host snapshot engine after shared profile data is cleared", () => {
    const registry = new HostSnapshotEngineRegistry();
    const firstHost = new FakeHostWebContents(1);
    const secondHost = new FakeHostWebContents(2);
    const firstEngine = registry.get(firstHost);
    const secondEngine = registry.get(secondHost);

    registry.clear();

    expect(registry.get(firstHost)).not.toBe(firstEngine);
    expect(registry.get(secondHost)).not.toBe(secondEngine);
  });
