...
import { BrowserAutomationExecuteRequestSchema } from "@thoth/protocol/browser-automation/rpc-schemas";
import type {
...
import {
  ...
  listRegisteredThothBrowserIdsForWorkspace,
  getThothBrowserWebContentsForHostWindow,
  getWorkspaceActiveThothBrowserIdForHostWindow,
...
export function clearBrowserAutomationSnapshotState(): void {
  hostSnapshotEngines.clear();
}
...
function createRegistry(hostWebContentsId: number): BrowserRegistry {
  return {
    listRegisteredBrowserIds: listRegisteredThothBrowserIds,
    listRegisteredBrowserIdsForWorkspace: listRegisteredThothBrowserIdsForWorkspace,
    getTabContents(browserId: string): TabContents | null {
      const contents = getThothBrowserWebContentsForHostWindow(browserId, hostWebContentsId);
      return contents ? adaptWebContents(contents) : null;
    },
    getBrowserWorkspaceId: getThothBrowserWorkspaceId,
    getWorkspaceActiveBrowserId(workspaceId: string): string | null {
      return getWorkspaceActiveThothBrowserIdForHostWindow(workspaceId, hostWebContentsId);
    },
  };
}
...
export function registerBrowserAutomationIpc(options?: { ipc?: IpcHandlerRegistry }): void {
  ...
  ipc.handle("thoth:browser:execute-automation-command", async (event, rawRequest: unknown) => {
