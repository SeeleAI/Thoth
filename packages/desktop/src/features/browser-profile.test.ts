...
describe("listThothBrowserProfileGuests", () => {
  test("returns every live webview and popup in the shared profile", () => {
    ...
    const guests = listThothBrowserProfileGuests({
      profileSession,
      webContents: [
        firstWindowGuest,
        secondWindowGuest,
...
describe("legacy browser profiles", () => {
  test("accepts only unique saved browser ids and resolves their old partitions", () => {
    ...
    const browserIds = readLegacyThothBrowserIds([uuid, fallbackId, uuid, "not-a-browser-id", 123]);
    const partitions: string[] = [];
    const sessions = getThothBrowserProfileSessions(
      {
    ...
    expect(partitions).toEqual([
      "persist:thoth-browser",
      `persist:thoth-browser-${uuid}`,
      `persist:thoth-browser-${fallbackId}`,
    ]);
  ...
  test("resolves one valid legacy profile for tab-close cleanup", () => {
    ...
    expect(getLegacyThothBrowserProfileSession(sessions, "1700000000000-abcd")).not.toBeNull();
    expect(getLegacyThothBrowserProfileSession(sessions, "invalid")).toBeNull();
    expect(partitions).toEqual(["persist:thoth-browser-1700000000000-abcd"]);
  });
});

describe("clearThothBrowserProfile", () => {
  test("clears site data, HTTP cache, and auth before reloading live guests", async () => {
    ...
    const clearing = clearThothBrowserProfile({
      profileSessions: [profile, legacyProfile],
      listGuests: () => {
        lifecycle.push("list-guests");
        return [firstGuest, secondGuest];
      },
      onCleared: () => lifecycle.push("invalidate-automation"),
      logReloadError: () => {},
    });
    ...
    expect(lifecycle).toEqual(["invalidate-automation", "list-guests"]);
  });
