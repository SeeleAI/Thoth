...
function getThothHome(): string {
  return resolveThothHome(process.env);
}

function logFilePath(): string {
  return path.join(getThothHome(), DAEMON_LOG_FILENAME);
}

export function isDesktopManagedDaemonRunningSync(): boolean {
  try {
    const raw = readFileSync(path.join(getThothHome(), "thoth.pid"), "utf-8");
