...
const mocks = vi.hoisted(() => ({
  thothHome: "/tmp/thoth-desktop-daemon-manager-test-home",
  settings: {
...
vi.mock("@thoth/daemon", () => ({
  resolveThothHome: vi.fn(() => mocks.thothHome),
...
describe("daemon-manager commands", () => {
  beforeEach(() => {
    ...
    rmSync(mocks.thothHome, { recursive: true, force: true });
  });
  ...
  it("starts the managed daemon detached from desktop stdio and reports daemon log failures", async () => {
    mkdirSync(mocks.thothHome, { recursive: true });
    writeFileSync(
      `${mocks.thothHome}/daemon.log`,
  ...
  it("passes stale lock reclaim only after a managed daemon is confirmed unreachable", async () => {
    mocks.runExternalCliJsonCommand.mockResolvedValue({
  ...
  it("does not authorize stale lock reclaim when the status probe itself fails", async () => {
