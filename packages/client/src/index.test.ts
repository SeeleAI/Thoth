...
import { createThothClient } from "./index.js";
import type { ThothAgent, ThothClient, ThothProviderConfig, ThothWorkspace } from "./index.js";
...
async function connectClient(): Promise<{ client: ThothClient; ws: FakeWebSocket }> {
  vi.stubGlobal("WebSocket", FakeWebSocket);
  const client = createThothClient({
    url: "ws://daemon.test",
  ...
  expect(hello.clientId).toEqual(expect.stringMatching(/^thoth-sdk-/));
  ws.message(
    sessionMessage({
      type: "status",
      payload: {
...
function createWorkspace(input: Partial<ThothWorkspace> = {}): ThothWorkspace {
  return {
    id: "workspace_sdk",
...
function createAgent(input: Partial<ThothAgent> = {}): ThothAgent {
  return {
    id: "agent_sdk",
    provider: "codex",
    cwd: "/repo/sdk",
...
test("createThothClient exposes workspace list through the daemon client", async () => {
