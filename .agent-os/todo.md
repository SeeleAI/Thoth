...
1. `NTH-TD-049` `[doing]`: Organically integrate the exact Paseo v0.2.3 target-side capability set, complete the Workspace-owned Schedule App surface, and replace the fixed desktop-only MVP Beta.
   - Scope: implement the approved Tasks/Schedules information architecture, Workspace-script semantic control, Provider usage/OMP/runtime ownership, atomic socket/E2EE/file-stream upgrade, strict service-proxy authority and local App/Desktop fixes through current Thoth owners; preserve all rejected/deferred boundaries and remove replaced paths during consumer cutover.
   - Acceptance: satisfy `NTH-AC-026`; require publish-mode provenance `36/36`, `12/12`, `4/4`, zero pending and zero failures before product-source edits; pass all focused/package/root/real-product gates; publish one exact release-source SHA through the existing fixed-Beta workflow; and reverify the exact 26 public desktop-only assets plus downloaded AppImage before completion.
   - Current result: implementation, focused/package suites, all five root gates, real Web, Android Debug APK, real Codex, isolation, hosted Relay v3 E2EE/multi-chunk transfer and rebuilt AppImage real-window acceptance pass. Production is `315,762` LOC and the formal boundary has zero blocking/review findings. The task remains doing until one release-source SHA passes both normal pushes, exact-SHA workflow, fixed 26-asset replacement and fresh public-AppImage verification.
   - Related: `NTH-REQ-029`, `NTH-AC-026`, `NTH-CD-089`, `NTH-CD-090`, `NTH-CD-091`, `NTH-CD-092`, `NTH-CD-093`, `NTH-CD-094`, `NTH-CD-095`, `NTH-CD-096`, `NTH-EV-072`, `NTH-TD-036`
