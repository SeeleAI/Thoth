...
1. `NTH-TD-048` `[doing]`: Organically integrate the approved capabilities and bug fixes in exact Paseo range `5fc53c57..b589599a`, verify every upstream commit and architecture candidate, and replace the fixed desktop-only MVP Beta from one exact source SHA.
   ...
   - Current result: local candidate is `release_ready`. All owner suites, provenance/boundary, five root gates, real Web/Codex, Android Debug APK, isolation and rebuilt AppImage Browser/Files/Changes/Schedule journey pass. Production is `310,932` LOC with approved `DeltaP=14,495`; publication and public-download reverification remain pending.
