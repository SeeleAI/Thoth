...
1. `NTH-TD-023` `[doing]`: Replace provider-session storage and split Loop authority with Workspace-sharded Task/Execution authority and the universal HarnessAdapter runtime.
   - Goal: Land `NTH-CD-060` as the only product path: deduplicated RuntimeBundles, provider-neutral adapters, one Workspace authority shard, unified Quick/Loop Tasks, Human Decision Ledger, Task Blackboard, same-Workspace `@Task`, fenced Card/Stop execution and one-shot installed-home migration.
   - Constraints: No provider-home copying, Codex business branch, old runtime fallback, dual read/write, hidden internal-Agent phase authority or acceptance-only product route. Existing `Simply Is First` edits remain intact. `main`, npm publication and Relay deployment stay out of scope.
   - Progress: The final modules and public protocols are implemented. Local adapter conformance, authority/migration, real Codex, fresh packaged AppImage, non-root packaged CLI over hosted Relay, Card restart continuation, Review fail/retry, `@Task`, Pause/Resume/Stop and storage checks pass. The remaining work is only the authorized branch pushes, native Actions matrix and re-downloaded public Release verification recorded under `NTH-EV-043`.
   - Acceptance: `NTH-AC-015`, adapter conformance for Codex/Claude/OpenCode/Pi/ACP, real Codex, fresh AppImage under ten-minute local acceptance, full release workflow and re-downloaded public artifact verification.
