...
1. `NTH-TD-054` `[doing]`: Repair history-only Provider capability detection and make the Decision Tree scene container-aware, collision-free and fully bounded, then replace the fixed desktop Beta.
   ...
   - Current result: focused Daemon recovery passes `3/3`; Decision Tree App tests pass `17/17`; Provider Control passes in `41.478s`; Decision Tree fast passes in `44.519s`; the final complete Thoth gate passes all `16` stages in `168.584s`; App/Daemon typechecks, Foundation, Web export and Release contract pass. The packaged workflow now asserts real DOM containment, conversation/tree separation, top alignment and the frozen Task leaf before publish. Commit, exact-SHA workflow and public verification remain pending.
