...
1. `NTH-TD-043` `[doing]`: Promote the current refactor intermediate state to `agent/dev/mvp` and replace the fixed desktop-only MVP Beta Release after complete local and native verification.
   ...
   - Current result: source-history comparison proves Cut 1 commit `26855ab7` introduced the Windows failure. The unified durability policy keeps file `fsync` plus atomic rename on every platform and skips only unsupported parent-directory `fsync` on Windows. Simulated Windows fresh/migrated storage passed `13/13`; source and packaged CLI cold starts passed; `accept:refactor:fast` passed in `151.055s`; the `1,408,855`-byte Server CLI package installed `278` packages and reached ready. Next: atomic commit, guarded normal pushes, exact-SHA Actions and public download verification.
   - Related: `NTH-CD-068`, `NTH-CD-069`, `NTH-CD-070`, `NTH-AC-022`, `NTH-EV-065`, `NTH-EXP-058`, `NTH-EXP-059`, `NTH-TD-036`, `NTH-REQ-026`
