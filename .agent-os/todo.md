...
1. `NTH-TD-054` `[doing]`: Repair history-only Provider capability detection and make the Decision Tree scene container-aware, collision-free and fully bounded, then replace the fixed desktop Beta.
   ...
   - Current result: source commit `89d07369` passes focused/fast/type/Foundation/Web gates. Exact-SHA workflow `31323481633` passed Windows, both macOS, Server CLI, three CLI smokes, Relay and preflight but failed the Linux packaged journey because its old whole-window assumption waited for a docked sidebar while the measured pane correctly chose overlay; publish was skipped and the old Release stayed intact. The corrected rebuilt AppImage journey now passes `ok=true` with `1000px` conversation, zero node overflow, full Task containment, `50.009px` top inset and a 900px overlay screenshot. Corrective commit, workflow and public verification remain pending.
...
29. `NTH-TD-052` `[verified]`: Replace Clarify and Loop with Decision Map cognition, one Intent Contract and a
    target-anchored Working Set architecture; migrate schema v5 to v6; and replace the fixed desktop Beta.

- Scope: remove Task/Goals runtime authority, linear Goal execution and Blackboard dumps; add same-session
  evidence-driven Clarify, one fresh Challenger, Task Anchor, Work Units, semantic checkpoints, fresh read-only
  Review, context reset and deterministic migration without compatibility paths.
- Related: `NTH-REQ-032`, `NTH-AC-029`, `NTH-CD-060`, `NTH-CD-100`, `NTH-CD-101`, `NTH-CD-102`, `NTH-EV-075`,
  `NTH-TD-036`
- Verification: See `NTH-EV-075`. Local owner/fast/real-Codex/judge/AppImage/Relay gates passed; failed workflows
  `30601495104`, `30602467471` and `30603411869` remained safe evidence; corrective source `c32ab051` completed all
