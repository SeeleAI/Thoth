...
1. `NTH-TD-043` `[doing]`: Promote the current refactor intermediate state to `agent/dev/mvp` and replace the fixed desktop-only MVP Beta Release after complete local and native verification.
   - Acceptance: preserve the open Cut B/final LOC targets honestly, repair the shared Windows daemon startup without a consumer-specific patch or fallback, pass focused storage/CLI/packaging gates, normally push only the two authorized branches, and require exact-SHA Actions plus public-asset/AppImage verification.
   - Current result: source-history comparison proves Cut 1 commit `26855ab7` introduced the Windows failure. Candidate `f50b9ea7` skipped unsupported directory `fsync`, passed all local gates, then exact-SHA run `30159851556` failed the same two Windows jobs while every other required job passed. The second correction opens the temporary file with write access before Windows `FlushFileBuffers` and persists early supervisor failures to `daemon.log`; targeted storage/logging passed `21/21`, daemon typecheck passed and a wholly fresh `accept:refactor:fast` passed in `149.010s`. Next: atomic commit, guarded normal pushes, exact-SHA Actions and public download verification.
   - Related: `NTH-CD-068`, `NTH-CD-069`, `NTH-CD-070`, `NTH-AC-022`, `NTH-EV-065`, `NTH-EXP-058`, `NTH-EXP-059`, `NTH-EXP-060`, `NTH-TD-036`, `NTH-REQ-026`
