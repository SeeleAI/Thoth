...
1. `NTH-TD-036` `[doing]`: Atomically converge AgentTimeline/tool rendering, Workspace/Sidebar responsive composition, Composer/Overlay, Settings/Panel/Card primitives and proven private UI glue into one final presentation path.
   ...
   - Current result: after the published Paseo integration, production is `310,933` LOC, `1,343,087` tokens, `1,362,659` AST nodes, `5,137` imports and `164` runtime dependency edges. The approved `DeltaP=14,496` translates the original `280,931` Cut B ceiling to `295,427` without counting new capability as refactor regression or progress. The remaining independent gap is exactly `15,506` LOC; Stage stays 4 and `NTH-TD-036` remains open.
